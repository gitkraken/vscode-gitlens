...
import { parseGitRefLog, parseGitRefLogDefaultFormat } from '../parsers/reflogParser.js';
...
export class CommitsGitSubProvider implements GitCommitsSubProvider {
	...
	async getIncomingActivity(
		...
		try {
			...
			const reflog = parseGitRefLog(result.stdout, repoPath, reflogCommands, limit, limit * 100);
			if (reflog?.hasMore) {
	...
	@debug({ exit: true })
	async getInitialCommitSha(repoPath: string, cancellation?: AbortSignal): Promise<string | undefined> {
