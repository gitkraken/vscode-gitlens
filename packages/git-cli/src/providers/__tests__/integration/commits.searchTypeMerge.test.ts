...
suite('CommitsSubProvider.searchCommits type:merge excludes stashes', () => {
	let repo: TestRepo;
	let mergeSha: string;
	...
	test('returns the merge commit but not stashes', async () => {
		...
		const { log } = await repo.provider.commits.searchCommits(repo.path, search);

		assert.ok(log?.commits.has(mergeSha), 'the real merge commit must be in the results');
		...
		for (const stashSha of stashList?.stashes.keys() ?? []) {
			assert.ok(!log?.commits.has(stashSha), `stash ${stashSha} must be excluded from type:merge results`);
		}
	});
