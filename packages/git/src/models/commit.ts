...
const previousShaCache = new WeakMap<GitCommit, Promise<string | undefined>>();
...
export class GitCommit implements GitRevisionReference {
	...
	static async getPreviousSha(commit: GitCommit): Promise<string | undefined> {
		let promise = previousShaCache.get(commit);
		if (promise == null) {
			promise = GitCommit.resolvePreviousSha(commit);
			previousShaCache.set(commit, promise);
		}
		return promise;
	}
	...
	private static async resolvePreviousSha(commit: GitCommit): Promise<string | undefined> {
		if (commit.file != null) {
			...
			const repo = getRepositoryService(commit.repoPath);
			const sha = (
				await repo?.revision.resolveRevision(
					isUncommitted(commit.sha, true) ? 'HEAD' : `${commit.sha}^`,
					commit.file.originalPath ?? commit.file.path,
				)
			)?.sha;
		...
		const repo = getRepositoryService(commit.repoPath);
		const sha = (await repo?.revision.resolveRevision(isUncommitted(commit.sha, true) ? 'HEAD' : `${commit.sha}^`))
			?.sha;
