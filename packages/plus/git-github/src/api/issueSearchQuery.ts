 ...
 *
 * A `sort:` qualifier is ALWAYS emitted — `criteria.sort` when given, `updated:desc` otherwise. Never none: the
 * provider serves at most a bounded window of matches, and without an explicit order GitHub answers in relevance
 * order, so which rows land inside that window would shift with GitHub's ranking even when nothing changed
 * upstream. A key GitHub can't express is refused before this by the facade, so it cannot arrive here.
 *
...
export function toGitHubIssueSearchQualifiers(criteria: IssueSearchCriteria | undefined): string[] {
	...
	const sort = toGitHubIssueSortQualifier(criteria?.sort ?? defaultIssueSort);
	if (sort != null) {
		qualifiers.push(sort);
	}
