...
import type { GraphCommitRef, GraphCommitView, RowRefOrder } from '../commit.js';
import { isRefHidden, isUpstreamRemoteOf, sortRowRefs } from '../commit.js';
...
import type { AutolinkIconStatus } from '../icons.js';
import { getAutolinkIcon } from '../icons.js';
import { refContextPinKey, refPillKey, remoteRefIcon } from '../refs.js';
import type { RowMarkerRole, RowMarkerTips } from '../rowMarkers.js';
import { primaryRowMarkerRole, rowMarkerRolesFor, shortRefName } from '../rowMarkers.js';
...
export interface RefPillHooks {
	...
	getHostContextAttribute?: () => string | undefined;
...
function renderPrChip(
	...
	return html`<span
		...
		${hostContext(hostContextAttribute, ref.context)}
		role="button"
		tabindex=${
...
function renderOnePill(
	...
	const hostContextAttribute = hooks?.getHostContextAttribute?.();
	const prChip = prMeta != null ? renderPrChip(prMeta.item, prMeta.ref, false, hostContextAttribute) : nothing;
	const prChipExpanded = prMeta != null ? renderPrChip(prMeta.item, prMeta.ref, true, hostContextAttribute) : nothing;
	const issueMeta = firstRefMetadata(hooks, (h, r) => h.getIssues(r), ref, upstreamOnRow);
	const issueChip =
		issueMeta != null ? renderIssueChip(issueMeta.item, issueMeta.ref, false, hostContextAttribute) : nothing;
	const issueChipExpanded =
		issueMeta != null ? renderIssueChip(issueMeta.item, issueMeta.ref, true, hostContextAttribute) : nothing;
...
function renderPopoverRefRow(
	...
	return html`<div
		...
		${hostContext(hooks?.getHostContextAttribute?.(), context)}
