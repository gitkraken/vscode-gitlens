...
import { pausedOperationVariantIcons } from '@gitlens/utils/pausedOperation.js';
import type { CommitGraphPausedOperationStatus } from '../../contracts/state.js';
import type { PausedOperationIndicatorInfo } from '../../rows/pausedOperation.js';
import { getPausedOperationIndicator } from '../../rows/pausedOperation.js';
 ...
 *     pill with a paused-op pill (icon + `getPausedOperationIndicator`'s label, e.g. "Rebasing" or "Resolve
...
function pausedOpFor(stats: WipStats): PausedOperationIndicatorInfo | undefined {
	return getPausedOperationIndicator(stats.pausedOpStatus, stats.hasConflicts === true, stats.conflictsCount);
}
...
function renderWipStatsBadge(stats: WipStats): TemplateResult {
	...
	const statsPill = html`<gl-wip-stats
		.added=${added || undefined}
		.modified=${modified || undefined}
		.removed=${deleted || undefined}
		.dirty=${added + modified + deleted > 0}
		.showClean=${true}
		.noTooltip=${true}
		icon="gl-worktree-filled"
	></gl-wip-stats>`;

	return html`<span class="gl-graph__wip-stats" style=${cspStyleMap(wrapStyle)}>${statsPill}</span>`;
}
...
function renderPausedOpPill(info: PausedOperationIndicatorInfo): TemplateResult {
