...
import { getAutolinkIcon, remoteRefIcon } from '@gitlens/components/components/icons/providerIcons.js';
...
import type { GraphCommitRef, GraphCommitView, RowRefOrder } from '../../rows/commit.js';
import { isRefHidden, isUpstreamRemoteOf, sortRowRefs } from '../../rows/commit.js';
import type { RowMarkerRole, RowMarkerTips } from '../../rows/markers.js';
import { primaryRowMarkerRole, rowMarkerRolesFor, shortRefName } from '../../rows/markers.js';
import { refContextPinKey, refPillKey } from './pills.js';
...
export interface ParsedRef {
	...
	/** JSON-stringified host context for this ref's pill. */
	context?: string;
}
...
export interface RefPillHooks {
	...
	/** The host's show-remote-names option — when false (the default), a remote pill's label is the bare
	 *  branch name instead of `remote/name`. Read fresh (the host's setting can change at runtime). */
	getShowRemoteNames: () => boolean;
	/** The host's max-inline-refs option — maximum number of ref pills shown inline per row before the rest
	...
	/** Id of the ref pinned to the edge (the host's pin-branch-to-edge option), read live so a pin/unpin shows
 ...
 * pin is deliberately not an ordering input); `cap` (the host's max-inline-refs option) only decides how many
...
function renderNamedUpstreamSegment(
	ref: ParsedRef,
	upstream: { name: string; hostingServiceType?: CommitGraphHostingServiceType; jumpSha?: Sha },
	hooks: RefPillHooks | undefined,
	iconOnly: boolean,
): TemplateResult {
...
// The host's show-remote-names option (default off): a remote pill is labeled with its bare branch name,
...
// there isn't. With the option ON the label is the explicit `origin/main` qualified form instead (a
