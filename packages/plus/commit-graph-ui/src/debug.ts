...
declare global {
	interface Window {
		 ...
		 */
		__gitkrakenCommitGraphDebug?: GraphDebugApi;
	}
}
...
const graphDebugPrefix = 'gitkraken.commitGraph.';
...
class GraphDebugDiagnostics {
	...
	constructor() {
		window.__gitkrakenCommitGraphDebug = {
...
/**
 * Lazily creates the development diagnostics. Every call site must sit behind the package's
 * `debugBuild` const (`typeof DEBUG !== 'undefined' && DEBUG`), never a bare `if (DEBUG)`: this package
 * ships plain `tsc` output, so a bare identifier would throw for any consumer whose bundler doesn't
 * define `DEBUG`. A host that defines it to `false` (GitLens does, via webpack) folds `debugBuild` to a
 * constant and the minifier drops both the guarded blocks and this module.
 */
