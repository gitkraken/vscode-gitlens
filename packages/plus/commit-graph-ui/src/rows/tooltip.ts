 ...
 *  `<gl-commit-graph>` as closures over itself; none of these run on a hot path. */
export type DelegatedTooltipHost = {
	...
	/** Closes the scroll-marker rail's own popover — it is a SEPARATE element, so showing this tooltip
	 *  doesn't replace it the way one shared popover used to. */
	hideScrollMarkerTooltip(): void;
};
...
export class DelegatedTooltipController implements ReactiveController {
	...
	showForTarget(target: HTMLElement): void {
		// The rail's popover is a separate element from ours — showing here must close it, or both stay up.
		this.deps.hideScrollMarkerTooltip();
