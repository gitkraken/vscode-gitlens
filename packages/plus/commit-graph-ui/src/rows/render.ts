...
import type { NodeStyle, WipNodeState } from '../gutter/render.js';
import { nodeRadiusFor, wipRingInnerRadius } from '../gutter/render.js';
...
export interface RowRenderContext {
	 ...
	 *  before the lane gutter — see graph.scss). See `gl-commit-graph.ts`'s `renderRowItem`. */
	...
	/** Per-`gl-commit-graph` memo over `renderGutterSvg` — the instance's gutter-template cache. Its epoch
	...
	/** Workdir-only: the host's action-strip descriptors for this row, in render order. Built once per
	 *  WIP-state change by the product, never per render. */
	wipActions?: readonly GraphRowAction[];
	 ...
	 *  when any. Drives the persistent, read-only paused-op/conflicts indicator. Any conflict-gated
	 *  INTERACTIVE action is the host's business — it ships one through {@link wipActions}. */
	wipPausedOpStatus?: CommitGraphPausedOperationStatus;
	/** Workdir-only, ALL worktrees (own + peer): whether this worktree has merge/rebase conflicts. Drives the
	 *  same read-only indicator as {@link wipPausedOpStatus} — a
	 ...
	 *  shown under the SAME decision (`gl-commit-graph.ts`'s `isPrimaryWipFarFromHead`). Never set for peer
...
// (`gl-commit-graph.ts`'s `computeWipRowFit`) is what actually keeps the content inside it.
...
function renderAvatar(
	...
	const avatarContext = ctx.avatarContextData ?? nothing;
 ...
 *  whole-strip hover/focus/selected fade. NOT simply `kind === 'workdir'` — a workdir row whose actions are
 *  all gated is JUST as hover-gated as a commit row.
 ...
 *  strip any more — the strip's persistence is driven solely by the host's descriptors below. */
/** Named rather than positional: the signature otherwise carries interchangeable `boolean | undefined`
 *  flags, and transposing any two of them at a call site type-checks clean. */
...
export function hasPersistentRowActions({
	...
	if (kind === 'workdir') {
		if (wipActions == null) return false;

		for (const action of wipActions) {
			if (action.persistent) return true;
		}
		return false;
	}
...
/** One host-supplied action button. `status` is either a code-icon name (rendered on the shared corner
 *  chip; `'loading'` spins) or a host template dropped in as-is. */
function renderWipRowAction(action: GraphRowAction): TemplateResult {
	const status =
		action.status == null
			? nothing
			: typeof action.status === 'string'
				? renderActionStatus(action.status, action.status === 'loading')
				: action.status;

	return html`<button
		class="gl-graph__row-action ${
			action.persistent ? 'gl-graph__row-action--persistent' : 'gl-graph__row-action--gated'
		}${action.className != null ? ` ${action.className}` : ''}"
		type="button"
		tabindex="-1"
		data-wip-open=${action.action}
		data-tooltip=${action.label}
		aria-label=${action.ariaLabel ?? action.label}
	>
		<code-icon icon=${action.icon}></code-icon>${status}
	</button>`;
}

// Row-action strip (right-aligned): per row kind — workdir renders the HOST's action descriptors
// (`ctx.wipActions`) verbatim, stash gets Apply/Drop, commit/merge gets Undo (leaf worktree tip) /
// Open-Changes / Push-to-Commit (unpushed). Buttons carry data-row-action / data-wip-open;
// gl-commit-graph's click delegation turns them into the gl-graph-rowaction / gl-graph-wiprowopen events
// the wrapper routes to the host. Which workdir buttons exist, what they mean and when they light up
// is the product's call — the kernel knows only the descriptor shape.
//
...
// strip fade. Persistent cases: a workdir descriptor that says so, the unpushed badge.
//
...
function renderRowActions(row: ProcessedGraphRow, ctx: RowRenderContext): TemplateResult {
	...
	switch (row.kind) {
		case 'workdir': {
			const wipActions = ctx.wipActions;
			hasPersistent = hasPersistentRowActions({ kind: row.kind, wipActions: wipActions });

			actions = html`${wipActions?.map(renderWipRowAction) ?? nothing}`;
			break;
		}
		case 'stash':
			actions = html`<button
...
export function renderRow(row: ProcessedGraphRow, ctx: RowRenderContext): TemplateResult {
	...
	// the row label instead. Reuses `getPausedOperationIndicator` from `pausedOperation.ts`, shared
	...
	// `wipMessageZoneStyle` below. `wipZoneSuppressFromIndex` is shared with `gl-commit-graph.ts`'s
