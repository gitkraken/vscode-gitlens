 ...
 * loading `lit`/`renderGutterSvg` (which reference `HTMLElement`). `renderGutterRaster` (graphGutter.ts)
...
// raster's inline `<defs>` (below) and the host's live `renderWavyFilterDefs` (graphGutter.ts, id
 ...
 *  transport-safe. Shared by the raster + the initials-dot builder (graphGutter.ts) so both serialize identically. */
