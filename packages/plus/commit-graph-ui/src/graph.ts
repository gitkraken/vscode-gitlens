...
import { createGraphDebugSnapshot, getGraphDebugDiagnostics } from './debug.js';
import type { LaneCollapseChipContext } from './extensions/laneCollapse/adornmentProvider.js';
import type { ParsedRef, RefPillHooks, RefPillRowMarker } from './extensions/refs/adornmentProvider.js';
import { getExcludedRemotes, refContextPinKey, refPillKey, resolveAutoRefPillCap } from './extensions/refs/pills.js';
import type { WipStats } from './extensions/wipStats/adornmentProvider.js';
import { GutterCache, gutterEpochSignature } from './gutter/cache.js';
import { ChainLaneOverlayController } from './gutter/chainLaneOverlay.js';
import '@lit-labs/virtualizer';
import { laneSeedKey, pickLaneSeed } from './gutter/laneSeed.js';
import type { WipNodeState } from './gutter/render.js';
import { laneSpacing, nodeRadiusFor, renderGutterSvg, renderWavyFilterDefs } from './gutter/render.js';
...
import type { RowMarkerTips } from './rows/markers.js';
import type { RowRenderContext } from './rows/render.js';
import { hasPersistentRowActions, renderChangesCellContent, renderRow, zoneLeadOffset } from './rows/render.js';
import { keepRowUnderRefVisibility } from './rows/rows.js';
import { DelegatedTooltipController } from './rows/tooltip.js';
import { RowUnitsIndex } from './rows/units.js';
import type { WipRowFit, WipRowFitMetrics, WipRowInfo } from './rows/wip.js';
import { computeWipRowFit, hasDirtyCounts, wipRowFullLabel, wipRowShortLabel } from './rows/wip.js';
import { countLoadedIncludedRefs } from './scope/paging.js';
...
// Fallbacks for the GROUPED inline lane cap when the host's `config.lanesGrouped*` options are absent:
...
const rowHasLocalHead: ScopeHeadsPredicate<CommitGraphSourceRow> = (row, branchName) =>
...
/** Why a loaded row isn't rendered — see {@link GlCommitGraph.getRowHiddenReason}. */
...
interface RenderCtx {
	...
	/** The host's `config.dimMergeCommits` option — when true, merge rows render dimmed. */
	...
	/** Product-supplied action-strip descriptors for each working-changes row. */
	rowActionsByRowSha?: ReadonlyMap<string, readonly GraphRowAction[]>;
	 ...
	 *  {@link GlCommitGraph.isPrimaryWipFarFromHead}. */
 ...
 * (source row → CommitGraphView → package-owned `CommitGraphEngineSession`), container-focus keyboard
 * nav, and the delegated interaction model. Emits the `gl-graph-*` events `<gl-graph-wrapper>` consumes.
 *
 * Light DOM (`createRenderRoot` returns `this`) so a host's native context-menu resolution works and
 * the renderer stylesheet applies.
 *
 ...
 */
export class GlCommitGraph extends LitElement {
	protected override createRenderRoot(): HTMLElement | DocumentFragment {
		return this;
	}

	@property({ type: Array }) rows?: CommitGraphSourceRow[];
	/** Immutable, one-time-prepared product profile. Extension slots are direct fields rather than a map. */
	@property({ attribute: false }) profile: CommitGraphProfile = minimalCommitGraphProfile;
	...
	// The host's action-strip descriptors per working-changes row — built once per WIP-state change,
	// rendered verbatim (see {@link GraphRowAction}).
	@property({ attribute: false }) rowActionsByRowSha?: ReadonlyMap<string, readonly GraphRowAction[]>;
	...
	// Host-serialized context-menu JSON strings (the single source of truth, matching the
	...
	// Branch pinned to the leftmost lane(s) (the host's pin-branch-to-edge option). Resolved to a sha and fed
	...
	// Backed by the host's `config.lanesFoldingEnabled` option (via the reactive `config` property).
	private get foldingEnabled(): boolean {
		return this.profile.laneCollapse != null && (this.config?.lanesFoldingEnabled ?? true);
	}

	private get stickyTimelineEnabled(): boolean {
		return this._stickyTimeline != null && this.config?.stickyTimeline !== false;
	}
	// Which lanes start folded, once folding is on — the host's `config.lanesFoldingDefault` option.
	...
	private readonly _stickyTimelineHost: StickyTimelineHost = {
		stickyTimelineEnabled: () => this.stickyTimelineEnabled,
		nowMs: () => this.nowMs,
	...
	// feature off. The selected extension constructs the controller once when its prepared profile arrives.
	private _stickyTimeline?: CommitGraphStickyTimelineController;
	private readonly _scrollMarkersHost: ScrollMarkersHost = {
		markerTypes: () => this.config?.scrollMarkerTypes,
		displayRows: () => this.displayRows,
		getCommit: sha => this.getCommitBySha(sha),
		...
		selectedShas: () => this.selectedShas,
		indexBySha: () => this.indexBySha,
		mergeTargetShas: () => this.mergeTargetShas(),
		mergeTargetName: () => this.rowMarkerMergeTarget?.name,
		pinnedSha: () => this.pinnedSha,
		pinnedName: () => this.pinnedRef?.name,
		totalRows: () => this._renderCtx?.total ?? this.displayRows.length,
		rowHeight: () => this._renderCtx?.rowHeight ?? 0,
		dataUnitOf: index => dataUnitForRow(this._rowUnits.unitsOf(index)),
		unitPositionOf: index => this._rowUnits.unitPosOf(index),
		viewportHeight: () => this.scrollerClientHeight,
		scroller: () => this.virtualizerRef.value,
		context: () => this.scrollMarkersContext,
		isDraggingColumn: () => this.draggingColumn,
		endRowHover: container => this.endRowHover(container),
		revealPosition: index => this.revealIndexAt(index, 0.5, true),
		jumpToRow: sha => this.jumpToRefRow(sha, { focus: true, flash: true }),
		hideDelegatedTooltip: () => this._tooltip.scheduleHide(),
	};
	private _scrollMarkers?: CommitGraphScrollMarkersController;
	...
	private refFindRef: Ref<GraphRefFinderElement> = createRef();
	...
	private renderRowItem(row: ProcessedGraphRow, index: number): TemplateResult {
		...
		// ABOVE it in display order (never row 0 — no "previous" to differ from). Gated on its OWN option
		// (the host's `config.timelineSeparators`), independent of the pill's `stickyTimeline` — this is the
		// FIRST condition in the `&&` chain, so it's a real short-circuit: disabled means zero
		// sticky-timeline group-key calls, not just a discarded result. Compares raw row dates (NOT
		...
		// visual: it says "this is uncommitted, everything below is history". The extension's group-key
		// function is allocation-free — no object/label/lo/hi built — and uses pure
		...
		const stickyTimelineGroupKey = this.profile.stickyTimeline?.groupKey;
		const isBucketBoundary =
			stickyTimelineGroupKey != null &&
			this.config?.timelineSeparators !== false &&
			row.date != null &&
			prevRowDate != null &&
			stickyTimelineGroupKey(row.date, this.nowMs) !== stickyTimelineGroupKey(prevRowDate, this.nowMs);
		...
		return renderRow(row, {
			...
			wipActions: row.kind === 'workdir' ? c.rowActionsByRowSha?.get(row.sha) : undefined,
			...
			// Every workdir row (own AND peer worktrees) gets these, driving the read-only paused-op/
			// conflicts indicator. Gating a conflict-driven INTERACTIVE action to the graph's own worktree
			// is the host's job — it ships that decision through `rowActionsByRowSha`.
			...
			avatarContextData: commit.avatarContextData,
	...
	private readonly engineSession = new CommitGraphEngineSession<CommitGraphSourceRow, GraphCommitView>();
	...
	private lastRowsRef?: CommitGraphSourceRow[];
	...
	// sha→HOST row map over `this.rows` (raw source rows, carries heads/remotes — `processedRows`
	...
	private cachedRowByShaRef?: CommitGraphSourceRow[];
	private cachedRowBySha?: ReadonlyMap<Sha, CommitGraphSourceRow>;
	...
	// Scroll-marker input identities stay on the surface because willUpdate already computes their
	// structural change signals. The optional controller owns all marker data, rendering, and interaction.
	...
	private lastLoadedRefIdsRowsRef?: CommitGraphSourceRow[];
	...
	private refsProvider?: RowAdornmentProvider<TemplateResult, ParsedRef[]>;
	private wipStatsProvider?: RowAdornmentProvider<TemplateResult, WipStats>;
	private lastProfile?: CommitGraphProfile;
	...
	private readonly scanVisibleRangeDebounced = debounce(
		...
		GlCommitGraph.wipSettleDelayMs,
	);
	...
	override willUpdate(changed: PropertyValues<this>): void {
		const profileChanged = this.profile !== this.lastProfile;
		if (profileChanged) {
			this.lastProfile = this.profile;
			this._stickyTimeline?.dispose();
			this._stickyTimeline = this.profile.stickyTimeline?.createController(this, this._stickyTimelineHost);
			this._scrollMarkers?.dispose();
			this._scrollMarkers = this.profile.scrollMarkers?.createController(this, this._scrollMarkersHost);
			this.refsProvider = this.profile.refs?.createProvider(
				() => this._refOrder,
				this.refPillHooks,
				() => ({
					excludeTypes: this.excludeTypes,
		...
		if (changed.has('config') || this.config !== this.lastConfigRef) {
			...
			if (!this.stickyTimelineEnabled) {
				this._stickyTimeline?.clear();
			} else if (this._stickyTimeline != null && !this._stickyTimeline.hasBucket) {
		...
		const wipChanged =
			profileChanged || this.wipStateById !== this.lastWipStateRef || this.repoPath !== this.lastWipRepoPath;
		if (wipChanged) {
		...
		const scrollMarkers = this._scrollMarkers;
		if (scrollMarkers != null) {
			...
			// invalidation — so it triggers the O(1) patch, never the row rescan.
			...
			if (baseMarkerInputsChanged || selectionChanged || mergeTargetChanged) {
				...
				scrollMarkers.recompute(!baseMarkerInputsChanged);
			}
		}
	...
	private updateRenderState(): void {
		...
		this._renderCtx = {
			...
			rowActionsByRowSha: this.rowActionsByRowSha,
			wipRowInfoByRowSha: this.wipRowInfoByRowSha,
	...
	private recomputeScope(): void {
		...
		const hasHead: ScopeHeadsPredicate<CommitGraphSourceRow> = (row, branchName) =>
	...
	// Resolve the pinned branch (the host's `pinnedRef` property) to a loaded sha: the host-provided
	...
	private resolvePinnedSha(rows: readonly CommitGraphSourceRow[]): string | undefined {
	...
	// stays zero-cost.
	private filterRowsByRefVisibility(rows: readonly CommitGraphSourceRow[]): readonly CommitGraphSourceRow[] {
	...
	private recomputeRows(idLength: number): void {
		...
		if (debugBuild && state.transition.kind !== 'append') {
	...
	// The host's `config.refsLayout` option (`'stacked'`): rows with real ref decorations grow a second unit (pills on top, commit
	...
	private readonly _tooltip = new DelegatedTooltipController(this, {
		...
		renderRefMetadataTooltip: (type, refId): TemplateResult | undefined => {
			if (refId == null) return undefined;

			const extension = this.profile.refs;
			const metadata = this.refsMetadata?.[refId];
			if (extension == null || metadata == null) return undefined;

			if (type === 'pullRequest') {
				const pullRequest = metadata.pullRequest?.[0];
				return pullRequest != null ? extension.renderPullRequestTooltip(pullRequest) : undefined;
			}

			const issue = metadata.issue?.[0];
			return issue != null ? extension.renderIssueTooltip(issue) : undefined;
		},
		hideScrollMarkerTooltip: (): void => this._scrollMarkers?.hideTooltip(),
	});
	...
	private rebuildWipStatsProvider(): void {
		const extension = this.profile.wipStats;
		if (extension == null) {
			this.wipStatsProvider = undefined;
			this.wipStateBySha = new Map();
			return;
		}
		...
		this.wipStatsProvider = extension.createProvider({ statsBySha: out });
	...
	private getRowByShaMap(): ReadonlyMap<Sha, CommitGraphSourceRow> | undefined {
		const rows = this.rows;
	...
	private rebuildProviders(): void {
		...
		const laneCollapse = this.profile.laneCollapse;
		if (this.foldingEnabled && laneCollapse != null) {
			...
			this.laneCollapseProvider = laneCollapse.createProvider({
				segmentsByTipSha: this.segmentsByTipSha,
				collapsedTips: this.effectiveCollapsed,
				hiddenCountByTipSha: this.hiddenCountByTipSha,
				branchHint: (tipSha: Sha) => laneCollapse.branchHintFor(rowBySha, tipSha),
			});
	...
	private get effectiveStyle(): ResolvedGraphStyle {
		// The host's `config.style` option wins when it forces a mode; `auto` (the default) switches to
	...
	// Lane-spacing density, driven by the host's `config.lanesDensity` option (via the `config`
	...
	// Resolves the host's `config.maxInlineRefs` cap: a configured number passes through (floored at 1);
	...
	private get effectiveMaxInlineRefs(): number {
		// The declared type is `number | 'auto'`, but the host's option schema admits ANY string — see
	...
	// Resolves the host's `config.maxStackedRefs` cap for a PROMOTED row's own pill line (row-units 'refs on
	// their own line' — `rowPromotesToOwnLine`): a configured number passes through as a FIXED cap (clamped
	// to [1, 10], same bounds as the option's schema); `'auto'` derives it from the refs-host zone's SOLVED
	// width via `resolveAutoRefPillCap` — the pills own their whole line up there, so an 'auto' cap sizes
	// against that width regardless of `config.maxInlineRefs` (that option is for the
	...
	// Date formatter honoring the host's `config.dateStyle` / `config.dateFormat` options, falling
	...
	private needsRelativeTimeTimer(): boolean {
		return this.isRelativeDateStyle() || this.stickyTimelineEnabled;
	}
	...
	override render(): TemplateResult {
		...
		return html`
			...
			>
				...
				${resultsBar}${this.renderStatusOverlay()}${this.renderChangesOptInOverlay()}${this._scrollMarkers?.render()}${this.renderWaypoints()}${this._stickyTimeline?.render()}${this.renderHScrollbar()}${this.renderChangesModePopover()}${this.renderRefFind()}
			</div>
			<span
				${ref(this.liveRef)}
	...
	/**
	 * "Jump to Pinned Branch" waypoint — shown only when a branch is pinned (the host's `pinnedRef` property)
	...
	private resolveRef(event: Event): ResolvedRefTarget | undefined {
		...
		// `context` is `data-vscode-context` for this SAME element (the pill root, a popover row, or a
		// PR/issue chip anchor all carry both together) — the host's double-click handler gates on
		// `ref.context` even for a metadata (PR/issue/upstream) click.
		return {
			name: name,
			key: key,
			kind: kind,
	...
	private rowKindForSha(sha: string): CommitGraphSourceRow['kind'] {
	 ...
	 *  keyboard-invoked row command has to carry, since there is no host-attribute DOM walk to do it.
	...
	private rowContextFor(sha: string | undefined): string | undefined {
		...
		if (worktreePath != null) {
			return this.profile.resolveWipRowContext?.(
				worktreePath,
	...
	private activatePill(refPill: ResolvedRefTarget, pillSha: string | undefined): void {
		...
		// The pill's own host context carries the refGROUP keys (the "Hide" action) merged in when this ref
		...
		this.dispatchEvent(
			new CustomEvent('gl-graph-open-branch', {
				detail: {
					...
					sha: pillSha,
					// Serialized host context for this ref — powers the sheet's kebab menu (row-menu
	...
	private onClick = (event: MouseEvent): void => {
		...
		// Row-action buttons (Open Changes / stash Apply-Drop / the host's workdir strip) resolve
		// BEFORE selection so a button click doesn't also select the row. They carry data-row-action
		// (→ the host's row-action channel) or data-wip-open (whose value is the host descriptor's own
		// `action`); the wrapper routes both. Alt on open-changes switches to the working-tree variant;
		// for a host descriptor the Alt state rides along in the event so the host can pick its alternate.
		for (const el of event.composedPath()) {
			...
			if (wipOpen != null) {
				...
				if (sha != null) {
					this.dispatchEvent(
						new CustomEvent('gl-graph-wiprowopen', {
							detail: { target: wipOpen, sha: sha, altKey: event.altKey },
						}),
					);
				}
				event.stopPropagation();
		...
		// Honor the host's `config.multiSelectionMode: false` — when multi-select is disabled, ctrl/shift/meta
	...
	// Right-click → emit the product context-menu event so the app can sync hover/selection
	// state. The host's native menu still opens on its own via the selected context attributes
	 ...
	 *  (ref metadata lives on the source row, not the engine's `ProcessedGraphRow`), no host round-trip. */
	...
	private _refVisibilityProbes?: {
		rows: CommitGraphSourceRow[] | undefined;
	 ...
	 *  `querySelector('gl-commit-graph')`) to pick the WIP sharing the clicked commit's visual lane. */
	...
	// `scrollToIndex(idx, 'nearest')` replacement that also honors the host's `config.scrollRowPadding` — rows of
	...
	private isScrollIdle(now: number = performance.now()): boolean {
		return this._lastScrollTime === 0 || now - this._lastScrollTime > GlCommitGraph.scrollIdleMs;
	}
	private onScroll = (event: Event): void => {
		...
		if (Math.abs(scrollTop - this._lastScrollTop) >= viewportH) {
			...
			if (now - this._lastTeleportTime < GlCommitGraph.scrollIdleMs) {
		...
		const stickyTimeline = this._stickyTimeline;
		if (stickyTimeline != null && this.config?.stickyTimeline !== false) {
			...
			stickyTimeline.updateFromScrollTop(scrollTop);
			...
			stickyTimeline.updateYield(scrollTop);
		}
	...
	private markScrolling(): void {
		...
		if (!this.clearScrolling.pending()) {
			this.endRowHover(null);
			this._tooltip.scheduleHide();
			// The rail's popover is anchored to a band that scrolling moves out from under the pointer.
			this._scrollMarkers?.hideTooltip();
		}
	...
	private buildWipRowBranchPill(row: ProcessedGraphRow): TemplateResult | undefined {
		...
		const parsed = refs.toParsedRefs([ref], this._refOrder);
		...
		let upstream: RefPillRowMarker['upstream'];
		...
		return refs.renderPill(parsed, colorForColumn(row.column), row.sha, this.refPillHooks, {
	...
	private buildWipRowFit(row: ProcessedGraphRow, c: RenderCtx): WipRowFit | undefined {
		...
		const availableWidth = GlCommitGraph.containerBudget(container);
		...
		for (const child of container.children) {
			...
			fixedWidth += GlCommitGraph.outerWidth(child);
		}
	...
	private checkWipFitConvergence(): void {
		...
		for (const [sha, usedBudget] of this._wipFitAvailableWidthBySha) {
			...
			if (usedBudget < 0 || Math.abs(GlCommitGraph.containerBudget(container) - usedBudget) > 1) {
				this.requestUpdate();
				return;
			}
		}
	...
	// re-derived for an arbitrary row OUTSIDE the render loop — a WIP row's host descriptors and a commit
	// row's unpushed/unpulled state live in plain fields/the payload plane, not just the per-render
	...
	private topRowHasPersistentActions(row: ProcessedGraphRow): boolean {
		const wipActions = row.kind === 'workdir' ? this.rowActionsByRowSha?.get(row.sha) : undefined;
	...
	private renderHeader(visibleZones: readonly ZoneSpec[], gutterWidth: number): TemplateResult {
		...
		>
			...
			${visibleZones.map((zone, i) => {
				...
				// A finder without `refs` would have nothing to jump to — gate here rather than let a
				// misconfigured profile reserve dead header space.
				const hasRefFinder = this.profile.renderRefFinder != null && this.profile.refs != null;
				...
				const refFindReservePx = hasRefsControl && hasRefFinder ? 22 : 0;
				...
				const refFindHere = hasRefsControl && hasRefFinder && headerW - baseControlsPx >= 44;
	...
	openRefFind(returnFocus?: HTMLElement): void {
		// Same guard `renderRefFind`/`hasRefFinder` use: without a finder to render, opening would push the
		// Esc overlay and strand focus on nothing, swallowing the next Esc.
		if (this.profile.renderRefFinder == null || this.profile.refs == null) return;
	 ...
	 */
	private renderRefFind(): TemplateResult | typeof nothing {
		if (this.profile.refs == null) return nothing;

		return (
			this.profile.renderRefFinder?.({
				elementRef: this.refFindRef,
				...
				onClick: this.onRefFindClick,
				onJump: this.onRefFindJump,
				onClose: this.onRefFindClose,
			}) ?? nothing
		);
	}

	private onRefFindClick = (event: Event): void => event.stopPropagation();
	...
	// Resolve the render style from the graph column `mode` + the host's `config.avatars` option. The
	...
	// Settings gear: opens the host's native graph menu (column show/hide + the Scroll Markers submenu)
	// on click. `settingsContext` is the host-built serialized context; a
	...
	private renderChangesLoading(
		...
		if (filterOnly && headerW < GlCommitGraph.changesSpinnerFilterOnlyFloor) return nothing;
	...
	// dedicated path (gl-commit-graph → graph-app → `columns.setColumnMode`).
	...
	// Settings-gear menu opener (sole consumer): dispatches a synthetic `contextmenu` at the gear so the
	// host's native menu opens there, resolving the gear's `settingsContext`. (The
	...
	// scoped under the `gl-commit-graph` selector rather than the row, so it inherits the real font/padding/gaps.
