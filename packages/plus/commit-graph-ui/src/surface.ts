...
import type { GraphCommitRef, RowRefOrder } from './commit.js';
import { columnsToZones, isRefHidden, pickGhostRef, zonesToColumnsConfig } from './commit.js';
import type { GraphRowActivity, GraphRowOperations } from './contracts/contributions.js';
...
import { ChainLaneOverlayController } from './graphChainLaneOverlay.js';
import { DelegatedTooltipController } from './graphDelegatedTooltip.js';
import '@lit-labs/virtualizer';
import type { FixedSizeLayoutSpecifier } from './graphFixedLayout.js';
import { fixedSizeVertical } from './graphFixedLayout.js';
import type { WipNodeState } from './graphGutter.js';
import { laneSpacing, nodeRadiusFor, renderGutterSvg, renderWavyFilterDefs } from './graphGutter.js';
import { GutterCache, gutterEpochSignature } from './graphGutterCache.js';
import type { RowRenderContext } from './graphRow.js';
import { hasPersistentRowActions, renderChangesCellContent, renderRow, zoneLeadOffset } from './graphRow.js';
...
import { getExcludedRemotes, refContextPinKey, refPillKey, resolveAutoRefPillCap } from './refs.js';
import { nearestNonWorkdirDate } from './rowDates.js';
import type { RowMarkerTips } from './rowMarkers.js';
import { keepRowUnderRefVisibility } from './rows.js';
import type { PreparedCommitGraphRuntime } from './runtime.js';
import { minimalCommitGraphRuntime } from './runtime.js';
import { countLoadedIncludedRefs } from './scopePaging.js';
import type { WipRowFit, WipRowFitMetrics, WipRowInfo } from './wip.js';
import { computeWipRowFit, wipRowFullLabel, wipRowShortLabel } from './wip.js';
import { hasDirtyCounts } from './worktree.js';
...
interface RenderCtx {
	...
	/** Product-supplied persistent activity for each working-changes row. */
	runningOperationByRowSha?: ReadonlyMap<string, GraphRowOperations>;
	activityByRowSha?: ReadonlyMap<string, GraphRowActivity>;
...
export class GlLitGraph extends LitElement {
	...
	@property({ attribute: false }) runtime: PreparedCommitGraphRuntime = minimalCommitGraphRuntime;
	...
	// Per-row WIP state for the row-action buttons: running operations and product-supplied activity.
	@property({ attribute: false }) runningOperationByRowSha?: ReadonlyMap<string, GraphRowOperations>;
	@property({ attribute: false }) activityByRowSha?: ReadonlyMap<string, GraphRowActivity>;
	...
	private get foldingEnabled(): boolean {
		return this.runtime.laneCollapse != null && (this.config?.lanesFoldingEnabled ?? true);
	}
	...
	// feature off. The selected extension constructs the controller once when its prepared runtime arrives.
	...
	private readonly _scrollMarkersHost: ScrollMarkersHost = {
		...
		hostContextAttribute: () => this.runtime.hostContextAttribute,
	...
	private renderRowItem(row: ProcessedGraphRow, index: number): TemplateResult {
		...
		const stickyTimelineGroupKey = this.runtime.stickyTimeline?.groupKey;
		...
		return renderRow(row, {
			...
			wipActivity: row.kind === 'workdir' ? c.activityByRowSha?.get(row.sha) : undefined,
	...
	private readonly refPillHooks: RefPillHooks = {
		getHostContextAttribute: () => this.runtime.hostContextAttribute,
	...
	private lastRuntime?: PreparedCommitGraphRuntime;
	...
	override willUpdate(changed: PropertyValues<this>): void {
		const runtimeChanged = this.runtime !== this.lastRuntime;
		if (runtimeChanged) {
			this.lastRuntime = this.runtime;
			this._stickyTimeline?.dispose();
			this._stickyTimeline = this.runtime.stickyTimeline?.createController(this, this._stickyTimelineHost);
			this._scrollMarkers?.dispose();
			this._scrollMarkers = this.runtime.scrollMarkers?.createController(this, this._scrollMarkersHost);
			this.refsProvider = this.runtime.refs?.createProvider(
				() => this._refOrder,
				this.refPillHooks,
				() => ({
					excludeTypes: this.excludeTypes,
		...
		const wipChanged =
			runtimeChanged || this.wipStateById !== this.lastWipStateRef || this.repoPath !== this.lastWipRepoPath;
		if (wipChanged) {
	...
	private updateRenderState(): void {
		...
		this._renderCtx = {
			...
			activityByRowSha: this.activityByRowSha,
			wipRowInfoByRowSha: this.wipRowInfoByRowSha,
	...
	private recomputeRows(idLength: number): void {
		...
		const state = this.engineSession.update({
			...
			toCommit: row => this.runtime.rowAdapter(row, idLength, this.repoPath, this.pinnedRef?.id),
	...
	private readonly _tooltip = new DelegatedTooltipController(this, {
		...
		renderRefMetadataTooltip: (type, refId): TemplateResult | undefined => {
			...
			const extension = this.runtime.refs;
	...
	private rebuildWipStatsProvider(): void {
		const extension = this.runtime.wipStats;
		if (extension == null) {
	...
	private rebuildProviders(): void {
		...
		const laneCollapse = this.runtime.laneCollapse;
	...
	private resolveRef(event: Event): ResolvedRefTarget | undefined {
		...
		// `context` is the host-serialized context for this SAME element (the pill
		...
		return {
			...
			remote: remote,
			context:
				this.runtime.hostContextAttribute != null
					? (el.getAttribute(this.runtime.hostContextAttribute) ?? undefined)
					: undefined,
	...
	private rowContextFor(sha: string | undefined): string | undefined {
		...
		if (worktreePath != null) {
			return this.runtime.resolveWipRowContext?.(
				worktreePath,
	...
	private buildWipRowBranchPill(row: ProcessedGraphRow): TemplateResult | undefined {
		...
		if (info.upstreamName != null && this.repoPath != null) {
			const upstreamId = `${this.repoPath}|remotes/${info.upstreamName}`;
	...
	// re-derived for an arbitrary row OUTSIDE the render loop — a WIP row's activity/operation status and a
	...
	private topRowHasPersistentActions(row: ProcessedGraphRow): boolean {
		const wipActivity = row.kind === 'workdir' ? this.activityByRowSha?.get(row.sha) : undefined;
	...
	private renderHeader(visibleZones: readonly ZoneSpec[], gutterWidth: number): TemplateResult {
		...
		>
			...
			${visibleZones.map((zone, i) => {
				...
				const hasRefFinder = this.runtime.renderRefFinder != null;
				...
				return html`<div
						...
						${hostContext(this.runtime.hostContextAttribute, this.columnsContext)}
						data-col-id=${zone.id}
	...
	private renderSettingsControl(): TemplateResult | typeof nothing {
		...
		return html`<button
			...
			${hostContext(this.runtime.hostContextAttribute, this.settingsContext)}
			type="button"
