...
import { buildRasterImageData, svgToDataUri, wavyFilterParams } from './raster.js';
...
export interface NodeStyle {
	...
	/** Reports a failed avatar image load — see `RowRenderContext.onAvatarError` (graphRow.ts). */
...
// `--gl-graph-font-family`, so the glyphs are shaped with the platform UI font (slight metric drift from the
...
export function renderGutterSvg(
	...
	// gutter cache offset-dependent (see `rowFadeable` in graphRow.ts).
