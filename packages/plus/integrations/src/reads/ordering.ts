 ...
 * read makes both decisions in the same shape, and before this they were spelled out four times.
 */
...
): { rejection?: 'unsupported-sort'; sort?: IssueSorting } {
	if (sort == null) {
		return supported?.includes(defaultIssueSort) === true ? { sort: defaultIssueSort } : {};
	}
	if (supported == null || !supported.includes(sort)) return { rejection: 'unsupported-sort' };
 ...
 */
export interface IssueOrdering {
	/** The key to send to the provider, or `undefined` to leave the provider's own order. */
	readonly sort: IssueSorting | undefined;
	/**
	 * True when the requested key cannot be honored: this page merges several queries, and no normalized issue
	 * carries the field to merge on. The read refuses rather than serving concatenated per-scope runs, which would
	 * look ordered without being so. Never true for a single-query page, where the provider did the ordering.
	 */
...
export function toIssueOrdering(sort: IssueSorting | undefined, merged: boolean): IssueOrdering {
	...
	return {
		sort: sort,
		unmergeable: merged && sort != null && comparator == null,
