...
import type { ProviderPullRequest, ProviderRepository } from '../providers/models.js';
import { createFakeRuntime } from './fakeRuntime.js';
import { primarySession, providerPr, stubApi } from './sweepHelpers.js';

/**
 * Bitbucket's account-wide pull request reads: the workspace fan-out and the reviewer slice it adds when
 * `includeReviewRequested` is set — page-prefix preservation, per-repo failure isolation, URL-less identity
 * dedupe, and the truncation each backstop reports (#5438).
 */

suite('Bitbucket account-wide pull request reads (#5438)', () => {
	...
	// Asserts a sweep-level default, so it reads out of place here — but the whole setup that makes it
	// meaningful is the Bitbucket reviewer fan-out above, and separating it from the slice it is about would
	// leave both halves harder to follow than the misfiled name suggests.
