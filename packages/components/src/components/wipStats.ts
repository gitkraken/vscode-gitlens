...
export class GlWipStats extends LitElement {
	static override styles: CSSResultGroup[] = [
		...
		css`
			...
			.wip-leading-icon {
				...
				color: var(--color-description-foreground, currentcolor);
	...
	private renderPausedOp(pausedOp: PausedOperationStatus): unknown {
		// Shares the bar's variant AND its label ladder so the header badge and the bar can't disagree.
		// Reading `pausedOperationStatusStringsByType` directly shares the string table but not the
		// decision on top of it, which is where they drift: the table has no `pending` entry, so a rebase
