...
import { parseDuration, waitForEvent } from '@gitlens/utils/dom.js';
   ...
   sitting on top of the border line */
	   ...
	   color — the body's own scrollbar thumb is handled separately below. */
			...
			:host(:focus-within) .popover__body {
				border-color: var(--gl-elevation-border-color, var(--color-contrast-border, transparent));
			}
			...
			:host([dragging]) .popover__resizer--active {
				background-color: var(--color-sash-hover-border, var(--color-focus-border, currentColor));
  ...
  body's visible border, which conflicts with the scrollableBase trick */
	   ...
	   would wrongly paint that tooltip with the menu background). */
			:host([appearance='menu']) .popover__body {
				...
				--wa-tooltip-font-size: var(--font-size, 12px);
				--wa-tooltip-background-color: var(--color-menu-background, var(--color-elevated-background));
			}

			:host([appearance='menu']) .popover {
				--arrow-color: var(--color-menu-background, var(--color-elevated-background));
			}
