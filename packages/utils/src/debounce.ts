 ...
 * @param {AbortSignal} options.cancellation - An optional AbortSignal to cancel the debounced function.
...
export function debounce<F extends (...args: any[]) => ReturnType<F>>(
	...
	let cancellation: DebounceOptions<F>['cancellation'];
	let aggregator: DebounceOptions<F>['aggregator'];

	if (options != null) {
		({ edges, maxWait, cancellation, aggregator } = options);
	}
