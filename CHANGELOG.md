...
- Adds a post-sign-in welcome to the _Commit Graph_ &mdash; completing sign-in from the graph's sign-in screen now lands on a _Welcome to the Commit Graph_ screen that introduces the graph, offers the Commit Graph walkthrough, links to set up AI, agents, and cloud integrations in _GitLens Settings_, and continues into the graph; signing in on the way to a specific task skips the welcome and returns to that task instead
- Adds task-specific messaging to the _Commit Graph_'s sign-in screen and Pro feature gate &mdash; arriving at either via a specific task (composing commits, an AI review, comparing references, or resolving conflicts), or losing access in the middle of one, now shows a headline and message for that task instead of the generic pitch, and regaining access returns to the interrupted task instead of a bare graph ([#5534](https://github.com/gitkraken/vscode-gitlens/issues/5534))
...
- Adds completed AI agent sessions to the _Commit Graph_ and _Home_ &mdash; sessions that have ended in the last 30 days now surface as de-emphasized _Completed_ rows alongside live ones, each with an _Archive Session_ action to dismiss it and an _Open Session_ action that resumes it in a terminal
...
- Adds HEAD, upstream, and merge target row markers to the _Commit Graph_ &mdash; the current branch's tip, its upstream, and its merge target now stand out with role-colored reference pills (HEAD in green and its upstream in a deeper green, since they're the same branch in two places, with the merge target as a jump segment on the pill), and each of those commits also gets a left-edge indicator that expands on hover; when the graph is focused (scoped) on a branch, that focus tip and its merge base share the same indicator rather than adding a second one; when a reference is pushed down the list (e.g. behind newer commits on other branches) the working changes (WIP) row surfaces the current branch's pill so its tip stays one click away; references that sit on the same commit (e.g. a branch in sync with its upstream) collapse into a single indicator; the merge target also gets its own marker on the graph's scroll bar &mdash; a full-width line alongside the existing HEAD and upstream markers, themeable via `gitlens.graphScrollMarkerMergeTargetColor`
- Adds account access to the _Commit Graph_ header &mdash; a new avatar pill after the _Launchpad_ indicator, ringed in a color that tracks your current entitlement and, when the header has room, carrying a short tier badge (e.g. `TRIAL`, `PRO`). Clicking the pill opens a rollup with your account summary, the active walkthrough &mdash; a completion ring with a count (e.g. 6/7) that follows the GitLens walkthrough until it's complete, then the _Commit Graph_ walkthrough, and disappears once both are done &mdash; and _AI_, _Agents_, and _Integrations_ sections showing what you have set up, or a _Set up…_ action when you don't; every one of them deep-links into the matching section of _GitLens Settings_. The _Show Account_ command (renamed _Show Account in Settings_) now opens the _Account_ section of _GitLens Settings_ instead of the _Home_ view ([#5522](https://github.com/gitkraken/vscode-gitlens/issues/5522))
...
- Adds a _Show Remote Branches_ toggle to the _Commit Graph_ side bar's _Branches_ panel &mdash; remote branches for the default remote can now be listed alongside local ones. Unlike the _Branches_ view, they keep their remote prefix (e.g. `origin/feature/x`), so they stay distinguishable in list layout and group under their remote in tree layout, and a remote branch is still listed when a local branch already tracks it; local branches always sort above remote ones. The toggle shares the `gitlens.views.branches.showRemoteBranches` setting with the _Branches_ view, so changing it in either place updates both. Remote branches also become pickable in the graph's _Focus Branch_ scope picker
- Adds a _Pull Requests_ panel to the _Commit Graph_ side bar &mdash; lists the repository's open pull requests most-recently-updated first, each with a trailing status indicator mirroring _Launchpad_'s grouping (mergeable, blocked, follow-up, needs review) on rows that need attention, a leading fork glyph on pull requests opened from a fork, and a hover adding that same grouping plus CI, review, and change-size signals — a fork's head named the way GitHub names one, `&lt;owner&gt;:&lt;branch&gt;`. Rows offer a state-aware first action &mdash; _Switch to Branch..._ (Alt: _Open in Worktree..._), or _Open Worktree in New Window..._ when one already exists, and neither when the pull request's branch is already the current checkout &mdash; then _Open Pull Request on Remote_ (Alt: _Copy Pull Request URL_), and _Focus_ (scopes the graph to the pull request's branch; when that branch isn't in your repository yet — one you haven't fetched, or a fork's head — it offers to fetch it first, adding a remote for the fork when needed). Right-click adds _Switch to Branch..._, _Open in Worktree_, _Open Pull Request Changes_, _Compare Pull Request_ (opens the graph's compare sheet on the pull request's base and head, instead of _Search & Compare_), _Open Pull Request_, and _Copy_ (`#<number> <title>`). When the repository's remote has no integration connected, the panel offers a _Connect to &lt;provider&gt;..._ action in place of the empty list &mdash; or a generic _Connect an Integration..._ one when the host isn't recognized (connecting a self-managed integration is what makes it recognizable) or the repository has no remotes yet &mdash; and connecting fills the list in without a reload

- Adds a type-ahead reference finder to the _Commit Graph_ &mdash; press `/` anywhere in the graph (outside of a text box), or use the search button in the references column header, and start typing to jump to a branch or tag by name. The graph moves as you type, the matched reference's pill fills and its row flashes briefly, and &uarr;/&darr; step through the other matches for when a short prefix doesn't land on the one you meant. Matching is by substring and requires every whitespace-separated term, and abbreviated path segments work too &mdash; `d/f/foo` finds `debt/feature/foo`. References whose commits aren't loaded yet are offered as well: they're named but dimmed, and pressing Enter fetches the commit and jumps to it, so typing and stepping never start a fetch on their own
- Adds contextual how-tos (coach marks) to the _Commit Graph_ &mdash; the first time you reach a feature, a short tip opens right where you are: commit details, _Compose_, _Review_, detected conflicts, _Resolve Conflicts_, the agents list, and _Compare_. Choose _Got it_ to dismiss it for good, or close it to park it on a lightbulb beside that feature to reopen any time. Where a tip belongs to a button &mdash; _Compose_, _Review_, _Resolve Conflicts_, _Compare_, and the _Agents_ side bar icon &mdash; that button also carries a "new" dot until you first interact with it ([#5516](https://github.com/gitkraken/vscode-gitlens/issues/5516))
- Adds repository consultation to AI-powered conflict resolution _(Preview)_ &mdash; when a conflict hunk is ambiguous on its own, AI can now read the whole file or either side at its ref, blame or log the lines in question, search the repo for other uses of the symbols involved, and diff the two sides before choosing a resolution, instead of deciding from the hunk's local text alone. Consultation is read-only and bounded &mdash; a step cap per conflict plus per-tool output caps &mdash; and is reported as it happens in both the _Commit Graph_'s **Resolve** panel and the automatic rebase, with each resolution's reasoning then citing what AI consulted to reach it &mdash; so the review says why, not just what; your AI file-exclusion rules (`gitlens.ai.exclude.files` and `.aiignore`/`.cursorignore`/`.aiexclude`) now apply to everything AI reads, not just the conflicted files. AI providers whose API can't carry tool calls fall back to the previous single-request behavior. This also fixes resolutions that needed a follow-up edit always pausing for review during an automatic rebase ([#5581](https://github.com/gitkraken/vscode-gitlens/issues/5581))
- Adds four sections to the _GitLens Settings_ page &mdash; _Account_, _Get Started_, _Agents_, and _Custom Remotes_ &mdash; and reorganizes the category rail into _Setup_, _Integrations_, _Views_, _Editor_, and _General_ groups ([#5392](https://github.com/gitkraken/vscode-gitlens/issues/5392))
  - _Account_ leads the rail &mdash; your account, plan, and subscription actions, or sign-in and create-account when signed out; its rail item carries your avatar when you're signed in
  - _Get Started_ is where a first open now lands, even though _Account_ sits above it in the rail, so you start on setup rather than billing &mdash; a launchpad of four steps (sign in, connect integrations, choose an AI provider and model, set up agents) with a completion ring tracking what's left, each step linking to the section that finishes it
  - _Agents_ lists every detected coding agent grouped by kind (chat, extension, CLI), each row with a _Default_ picker and, for CLI agents, install controls for GitKraken MCP and Claude Hooks; these rows move out of the _AI_ section into their own
  - _Custom Remotes_ configures the `gitlens.remotes` array in the UI instead of by hand-editing JSON &mdash; one collapsed row per rule summarizing its type and matcher, expanding into a domain-or-regex matcher, name, protocol, and ignore-SSL, plus the URL template block, with per-field token hints, that fully custom hosts need
...
- Adds an automatic rebase that resolves conflicts end-to-end with AI &mdash; choose _Automatic Rebase_ in the rebase quick menu to resolve, stage, and continue each conflicted step, pausing for review when confidence falls below `gitlens.ai.autoRebase.confidenceThreshold`. A per-step summary shows every file's strategy, confidence, and reasoning, with a validated _Undo_ ([#5450](https://github.com/gitkraken/vscode-gitlens/issues/5450))

### Changed

- Changes the _Commit Graph_'s default home to the GitLens side bar, where it now sits above _Home_ as the main view &mdash; only the default changes, so if you moved the Graph yourself it stays exactly where you put it, and a one-time prompt offers the bottom panel (or you can just drag the view). At the side bar's default width the graph's `auto` modes kick in &mdash; compact list-style rows and the details panel below the graph; widen or move the view for the columnar table layout. The _Prefer Commit Graph in Panel_ command is now _Prefer Commit Graph as a View_; the `gitlens.graph.layout` setting is unchanged ([#5545](https://github.com/gitkraken/vscode-gitlens/issues/5545))
- Changes the _Toggle Commit Graph_ command to hide the Graph view itself rather than closing the bottom panel, so it works wherever the view lives; _Toggle Maximized Commit Graph_ now just reveals the Graph — its former panel-maximize has no side bar equivalent — and leaves the Command Palette (it stays available for existing keybindings &mdash; search "Toggle Maximized Commit Graph" in Keyboard Shortcuts, as it no longer carries the GitLens category prefix) ([#5545](https://github.com/gitkraken/vscode-gitlens/issues/5545))
- Changes the _Home_, _Cloud Patches_, and _Cloud Workspaces_ views to be hidden by default &mdash; applied once on upgrade too (landing you on the Graph in its new home); each remains available from the Command Palette (_GitLens: Show Home View_, _Show Cloud Patches View_, _Show Cloud Workspaces View_) and stays visible once shown ([#5545](https://github.com/gitkraken/vscode-gitlens/issues/5545))
...
- Changes paused-rebase auto-open (`gitlens.rebaseEditor.openOnPausedRebase`) to only open for rebases you're driving from GitLens &mdash; a new `auto` value, now the default, ignores paused rebases started outside GitLens (terminals, agents, other tools), which previously popped open the _Interactive Rebase Editor_ or _Commit Graph_ uninvited; rebases started from GitLens (rebase commands, the _Commit Graph_, automatic rebase, or a pull that rebases) still open automatically, as does an external rebase once you act on it through GitLens (e.g. _Continue_); set it to `true` to restore the previous open-for-any-rebase behavior ([#5686](https://github.com/gitkraken/vscode-gitlens/issues/5686), [#4954](https://github.com/gitkraken/vscode-gitlens/issues/4954))
- Changes the _Commit Graph_ sidebar to be unpinned by default &mdash; the sidebar now floats over the graph and auto-collapses when it loses focus or when you press Esc; pin it (or set `gitlens.graph.sidebar.pinned` to `true`) to restore the previous shared-space layout ([#5447](https://github.com/gitkraken/vscode-gitlens/issues/5447))
...
- Changes the _Commit Graph_ minimap to be hidden by default and shown while searching &mdash; it takes up real vertical space without always being relevant, so when to show it now lives in a new `gitlens.graph.minimap.defaultVisibility` setting: `"onSearch"` (the new default), `"always"`, or `"hidden"` (available, but only shown when you ask for it). On `"onSearch"` the minimap appears as soon as a search starts, so matches are visible as they stream in, and hides again once the search is cleared; hiding it during a search dismisses it for that search only, while showing it when no search is running keeps it up for that workspace. `gitlens.graph.minimap.enabled` goes back to a plain boolean controlling whether the minimap is available at all &mdash; when disabled it is never shown and its header toggle is removed. An existing `false` is honored unchanged; an existing `true` now means available and shown while searching, so set `gitlens.graph.minimap.defaultVisibility` to `"always"` to keep it always shown ([#5598](https://github.com/gitkraken/vscode-gitlens/issues/5598))
...
- Changes paused merge, rebase, cherry-pick, and revert conflicts to surface in one place &mdash; the _Commit Graph_'s working changes (WIP) details, or the _Interactive Rebase Editor_ for rebases when the graph isn't usable; a rebase conflict previously opened the editor, force-focused the _Commits_ view, and showed a redundant toast all at once
...
- Fixes AI conflict resolution comparing the incoming side against itself &mdash; the prompt's three-way diff, which the model is told to prefer over inspecting the repository, showed the same side as both "current" and "incoming" for every conflict (in a rebase, merge, cherry-pick, or revert), so it read as though both sides had made the identical change instead of describing what each side actually did. Resolutions now see what genuinely differs between the two sides
...
- Fixes GitLens needing a window reload to notice when you trust a workspace &mdash; after granting trust to a folder opened in Restricted Mode, Git operations kept failing as if the workspace were still untrusted (repositories weren't discovered and views like the _Commit Graph_ and _Home_ stayed empty) until the window was reloaded; the trusted state is now re-read live, so repositories are discovered as soon as you grant trust
...
- Fixes working tree (WIP) search results (`type:wip`) all being marked at today on the _Commit Graph_'s minimap &mdash; each worktree's WIP row is now marked at the commit it's anchored to, matching where that worktree's marker is already drawn, rather than stacking every worktree onto today; selecting or hovering a WIP row now highlights that same day. The primary working changes row still sits at the start of the timeline, which is where its row is
- Fixes clearing a natural language search in the _Commit Graph_ silently re-populating the search box a moment later &mdash; a query still being converted when the search was cleared went on to run and restore itself, so the box would empty and then refill with the converted query
- Fixes the paused-operation bar offering an enabled _Continue_ while Git was still waiting for you to save and close the commit message tab &mdash; the operation looked stuck, with the click swallowed, no progress, and no hint of what it was waiting for. The bar now holds an in-progress state until the operation actually settles, including when it's continued from another surface and when the operation belongs to a worktree ([#5656](https://github.com/gitkraken/vscode-gitlens/issues/5656), [#5668](https://github.com/gitkraken/vscode-gitlens/issues/5668))
- Fixes the paused-rebase banner (in the _Commit Graph_, _Inspect_, and _Home_) showing two identical play icons once AI has resolved the conflicts &mdash; _Continue_ and _Continue with AI_ shared the same glyph, so only their tooltips told them apart, and while conflicts were still unresolved the AI action was the lone play icon despite resolving conflicts rather than just continuing. The AI action now uses a sparkle-badged continue icon and is renamed _Continue Automatic Rebase_ to match the command of the same name, and the plain action now names its operation (e.g. _Continue Rebase_); the _Continue Automatic Rebase_ context menu entry in the views also picks up the new icon in place of the merge icon it was using ([#5450](https://github.com/gitkraken/vscode-gitlens/issues/5450))
- Fixes _Recompose_ failing outright when branches or tags point at commits inside the range being recomposed &mdash; it now warns that those references won't follow the new history (they keep pointing at their current commits, and nothing is lost) and recomposes if you continue ([#5515](https://github.com/gitkraken/vscode-gitlens/issues/5515))
...
- Fixes the _Commit Graph_'s compose preview being misaligned for the second and later proposed commits, showing their changes spliced into the wrong lines ([#5596](https://github.com/gitkraken/vscode-gitlens/issues/5596))
- Fixes the _Commit Graph_'s compose preview for a later proposed commit dropping earlier hunks of a file renamed with edits, so it looked like that commit re-introduced content an earlier one had removed. Preview only &mdash; applying the compose was unaffected ([#5606](https://github.com/gitkraken/vscode-gitlens/issues/5606))
- Fixes excluding an untracked nested repository from a compose having no effect &mdash; its entry was still sent to AI and still committed, rather than being left in your working tree ([#5611](https://github.com/gitkraken/vscode-gitlens/issues/5611))
- Fixes an untracked nested repository's row having no name in _Files Changed_ &mdash; both its label and its include/exclude tooltip were blank; the row now names the directory and marks it as a nested repository on hover ([#5629](https://github.com/gitkraken/vscode-gitlens/issues/5629))
- Fixes files you excluded from a review being able to reach AI anyway &mdash; a review of a focus area cached the diff before applying your exclusions, and a later review reused that cached diff as though it were already filtered ([#5630](https://github.com/gitkraken/vscode-gitlens/issues/5630))
- Fixes _Copy Changes (Patch)_ producing an empty or incomplete patch &mdash; copying a multi-file working tree (WIP) selection in the _Inspect_ and _Commit Graph_ views diffed against the index rather than HEAD, so a fully-staged selection reported "No changes found to copy" and a selection mixing staged and unstaged files silently dropped its staged changes; renamed files also copied as a bare add of the new path (leaving the old file behind when the patch is applied) because the patch's file filter omitted the original path &mdash; this affected every per-file _Copy Changes (Patch)_ entry point, including commits, comparisons, the views, and the _Source Control_ view
- Fixes AI conflict resolution (**Resolve** mode in the _Commit Graph_) failing with _No active merge, rebase, or cherry-pick conflicts to resolve_ when conflicts exist without a paused Git operation, such as after a `git stash pop` or an autostashed pull ([#5487](https://github.com/gitkraken/vscode-gitlens/issues/5487))
- Fixes the walkthrough's _AI Features_ step &mdash; and the _Welcome_ view's matching "Commit smarter, not harder" section &mdash; never completing from the very flow they promote: their _Compose Commits_ button leads to the _Commit Graph_'s **Compose** panel, but generating commits there didn't count toward the step; composing commits with AI now marks it complete
