...
- Changes the _Commit Graph_'s default home to the GitLens side bar, where it now sits above _Home_ as the main view &mdash; existing users are moved there once on upgrade, and the layout prompt then offers to send it back to the bottom panel for good (choosing the bottom panel keeps that container in the bottom panel, while choosing the side bar leaves the GitLens container wherever you keep it). The _Prefer Commit Graph in Panel_ command is now _Prefer Commit Graph in Side Bar_; the `gitlens.graph.layout` setting is unchanged ([#5545](https://github.com/gitkraken/vscode-gitlens/issues/5545))
- Changes the _Home_, _Cloud Patches_, and _Cloud Workspaces_ views to be hidden by default &mdash; each remains available from the Command Palette (_GitLens: Show Home View_, _Show Cloud Patches View_, _Show Cloud Workspaces View_) and stays visible once shown ([#5545](https://github.com/gitkraken/vscode-gitlens/issues/5545))
