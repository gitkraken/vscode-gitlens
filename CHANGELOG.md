...
- Changes paused merge, rebase, cherry-pick, and revert conflicts to surface in a single place &mdash; the _Commit Graph_'s working changes (WIP) details, or the _Interactive Rebase Editor_ for rebases when the graph isn't usable (gated, or signed out, where it shows only the sign-in screen). Previously a rebase conflict could open the rebase editor, force-focus the _Commits_ view, and show a redundant toast all at once. Interactive rebase pauses (todo editing, `edit`, `reword`, `break`) still open the rebase editor
