...
- Adds a bright lane highlight when pinning a branch's ref pill in the _Commit Graph_, and to the `Ctrl`-held chain highlight &mdash; the branch's lane stays lit through dimmed rows and traces down to its fork point, while other lanes mute so the highlighted one stands out
