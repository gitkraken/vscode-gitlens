...
- Adds task-specific messaging to the _Commit Graph_'s sign-in screen &mdash; arriving there via a specific task (composing commits, an AI review, comparing references, or resolving conflicts), or being signed out in the middle of one, now shows a headline and message for that task instead of the generic pitch, and signing back in returns to the interrupted task instead of a bare graph ([#5534](https://github.com/gitkraken/vscode-gitlens/issues/5534))
