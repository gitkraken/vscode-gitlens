...
- Improves the flow of connecting agents from the _Commit Graph_'s _Agents_ side bar panel ([#5777](https://github.com/gitkraken/vscode-gitlens/issues/5777)) &mdash; when no agents are connected, the panel now explains why it is empty instead of showing a generic "No items", with a _Connect Agents..._ action that opens the _Agents_ page in _GitLens Settings_ and, when a default agent is set, also installs its hooks right away (_Manage Agents..._ without a default); adds a _Start Agent Session..._ header action (hold `Alt` to always pick the agent)
...
- Adds an evidence-based _Repository Health_ banner to the _Commit Graph_ &mdash; when a repository has suggested optimizations and Git has been measurably slow there (or the repository is clearly large), a dismissible strip above the graph explains what was noticed, shows how many optimizations are suggested, and opens _Repository Health_ in one click; the side bar's _Visualizations & Health_ toggle carries an indicator dot until the health view is visited, and its tooltip reports the suggested-optimization count for as long as any remain
...
- Adds live agent status for more coding agents &mdash; Codex, GitHub Copilot CLI, and OpenCode sessions now appear alongside Claude Code's everywhere agent status is shown (the _Commit Graph_'s _Agents_ side bar panel, working-changes and branch details, and _Home_), each carrying its own icon so sessions from different agents stay distinguishable; GitKraken Hooks can now be installed for them from the _Agents_ table in _GitLens Settings_ and from the install-all commands ([#5742](https://github.com/gitkraken/vscode-gitlens/issues/5742), [#5798](https://github.com/gitkraken/vscode-gitlens/issues/5798))
  - Surfaces the extra step an agent's own host requires before installed hooks take effect &mdash; Codex will not run hooks until you review and trust them with `/hooks` inside Codex, so the install notification and the _Agents_ settings row now say so and offer to start a Codex session in a terminal to do it (trust is tied to the hooks themselves, so reinstalling them asks again)
  - Shows permission requests from these agents without _Allow_ / _Deny_ actions, since only Claude Code can be answered from GitLens today; resuming past sessions and transcript-derived session names likewise remain Claude Code only
...
- Changes the _Commit Graph_'s details toggle into a split button ([#5747](https://github.com/gitkraken/vscode-gitlens/issues/5747)) &mdash; the toggle still shows and hides the panel with one click (and no longer resets `gitlens.graph.details.location` to `auto` in your settings when the panel closes), while a new chevron opens a placement menu with _Auto_, _Right_, and _Bottom_ thumbnails; the _Auto_ thumbnail previews which side it would pick for the current window shape, and choosing a placement while the panel is hidden also shows it
...
- Fixes opening changes for a modified tracked file from a worktree's _Working Changes_ row failing with "the editor could not be opened because the file was not found" &mdash; when the worktree lives inside another repository's folder ([#5773](https://github.com/gitkraken/vscode-gitlens/issues/5773))
- Fixes commit message actions targeting the containing repository for a worktree that lives inside another repository's folder &mdash; generating a commit message, restoring an undone commit's message, and _Commit via SCM_ wrote to or focused the wrong _Source Control_ input box ([#5773](https://github.com/gitkraken/vscode-gitlens/issues/5773))
- Fixes the _Commit Graph_'s _Working Changes_ details collapsing its file list to zero rows and clipping the commit box behind a scrollbar when the details panel is short &mdash; e.g. in the side bar graph; the panel now refuses to size below its content and keeps at least one file row visible
...
- Fixes enriched autolinks being cached without bound for the life of the session &mdash; each viewed commit's message was retained as a cache entry forever, and compare ranges retained every message joined into a single entry; entries now expire after 30 minutes with a capped entry count ([#5504](https://github.com/gitkraken/vscode-gitlens/issues/5504))
- Fixes the account popover advertising out-of-date GitKraken AI allowances &mdash; it offered Pro at "250K tokens/week" (really the trial grant) and Advanced at "1M tokens/week" (really Pro's); both now read the correct per-plan figure in credits &mdash; Pro 1M and Advanced 2M per week
...
- Adds paused operation status to working tree (WIP) rows in the _Commit Graph_ &mdash; a worktree stopped partway through a rebase, merge, cherry-pick, or revert now reads as that operation in place of its working-changes count, including worktrees other than the one you have open, and unresolved conflicts surface even when no operation is recorded
- Adds double-click-to-scope to the _Commit Graph_ side bar and overview bar &mdash; double-clicking a branch row or pill focuses the graph on that branch, and double-clicking it again clears the scope; the single-click behavior still runs, held briefly so the second click can take over
...
- Fixes GitLab self-hosted repo-scoped reads (issues and pull requests) 404ing with every request &mdash; `getSelfManagedApiBaseUrl` was appending `/api` to the base URL passed to `provider-apis`, which then appended another `/api/v4`, causing 404s; it now correctly strips redundant API segments so the final URL is correct ([#5526](https://github.com/gitkraken/vscode-gitlens/issues/5526))
