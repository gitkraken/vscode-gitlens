...
- Changes the _Commit Graph_ minimap to be hidden by default and shown while searching &mdash; it takes up real vertical space without always being relevant, so `gitlens.graph.minimap.enabled` now accepts `"auto"` (the new default) alongside `true` (always show) and `false` (never show). On `"auto"` the minimap appears as soon as a search starts, so matches are visible as they stream in, and hides again once the search is cleared; hiding it during a search dismisses it for that search only, while showing it when no search is running keeps it up for that workspace. Any existing explicit `true` or `false` setting is honored unchanged ([#5598](https://github.com/gitkraken/vscode-gitlens/issues/5598))
