...
- Adds an empty state to the _Commit Graph_ when no repository is open &mdash; a _Get Started_ group offers _Open a Folder_, _Clone a Repository_, and _Start a New Project_ actions (in web/virtual environments, e.g. vscode.dev, clone and new-project are replaced by an _Open Remote Repository_ action), and a _Start From Your Work_ group offers _Start Work on an Issue_ and _Start Review on a PR_ (desktop-only) actions that pick from your connected integrations and offer to clone or locate the repository when it isn't available locally ([#5408](https://github.com/gitkraken/vscode-gitlens/issues/5408))
...
- Fixes the paused-operation bar in _Home_ offering an enabled _Continue_ again about a second after it was clicked, while Git was still waiting for you to save and close the commit message tab &mdash; the bar's in-progress state was cleared by the next overview refresh rather than by the command ending, so it reported the click as lost and contradicted the _Commit Graph_, which stayed busy for the same operation. _Home_ now takes the same signal the graph does, so both hold their in-progress state until the operation actually settles &mdash; including when it's continued from another surface, and when the operation belongs to a worktree ([#5668](https://github.com/gitkraken/vscode-gitlens/issues/5668))
