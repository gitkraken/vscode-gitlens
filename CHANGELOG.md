...
- Adds an all-new high-performance rendering engine on the _Commit Graph_, rewritten from the ground up &mdash; loading, rendering, updating, and scrolling are significantly faster, it uses substantially less memory, and it is far more keyboard and screen reader accessible
  - Lanes now hold still &mdash; fetches and newly arriving commits no longer reshuffle lanes or their colors, history rewrites (a rebase, amend, or reset) re-lay out cleanly instead of dragging lanes out of place, the viewport stays parked on the commit you're reading as history arrives above it, and a new _Quick Refresh_ button in the column header re-lays out the lanes on demand
