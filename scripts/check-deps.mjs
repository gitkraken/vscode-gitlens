...
async function main() {
	...
	await checkGeneratedDependencies(errors);
...
// Manifests whose `dependencies` are generated by a bundle script, paired with the function that
// recomputes them. Nothing regenerates these when a bundled package gains or drops a dependency, so
// a stale copy would otherwise ship dependencies that no longer match the code in the tarball.
const generatedDependencies = [
	{ member: 'packages/core', script: 'pnpm run build:core', compute: mergeBundledDependencies },
];

/** @param {string[]} errors */
async function checkGeneratedDependencies(errors) {
	for (const { compute, member, script } of generatedDependencies) {
		...
		try {
			...
			errors.push(`${member}/package.json could not be read: ${ex.message}`);
			continue;
		}
		...
		if (stale.length) {
			errors.push(`${member}/package.json is stale — run \`${script}\` to regenerate it. ${stale.join('; ')}.`);
		}
	}
}
