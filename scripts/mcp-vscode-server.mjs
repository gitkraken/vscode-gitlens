...
async function queryAllFrames(currentPage, selector, action = 'text') {
	...
	for (const frame of currentPage.frames()) {
		const url = frame.url();
		if (url === 'about:blank' || url === currentPage.url()) continue;
		try {
...
// =============================================================================
async function connectEvaluator(app) {
...
// =============================================================================
// Find webview frame by title (searches nested webview iframes)
// =============================================================================
async function findWebviewFrame(webviewTitle, extensionId) {
	requireReady();
	for (const frame of page.frames()) {
		...
		// Skip non-webview frames
		if (url === 'about:blank' || url === page.url()) continue;
		// Check if this frame or its child frames match the title
		try {
	...
	// If not found by title on frames directly, try the nested iframe pattern:
	// outer iframe.webview -> inner iframe#active-frame
	for (const frame of page.frames()) {
		const url = frame.url();
		if (url === 'about:blank' || url === page.url()) continue;
		try {
			// Check if any text content in the frame matches as a heuristic
			const bodyText = await frame
				.locator('body')
				.textContent({ timeout: 500 })
				.catch(() => '');
			if (bodyText) return frame; // Return first non-empty frame if title matching fails
		} catch {}
	}
	return null;
}
	...
	async args => {
		...
		try {
			if (args.target === 'webview' && args.webview_title) {
				...
				const frame = await findWebviewFrame(args.webview_title, sessionConfig.extensionId);
				if (frame) {
					// Screenshot the frame's owner element if possible
					try {
		...
		try {
			if (args.in_webview || args.webview_title) {
				if (args.webview_title) {
					const frame = await findWebviewFrame(args.webview_title, sessionConfig.extensionId);
					if (frame) {
						await frame.locator(args.selector).first().click({ timeout: 3000 });
		...
		try {
			if (args.in_webview || args.webview_title) {
				...
				if (args.webview_title) {
					const frame = await findWebviewFrame(args.webview_title, sessionConfig.extensionId);
					if (frame) {
						const results = await extractFromLocator(frame.locator(args.selector), prop, maxResults);
						return textResult(
							results.length > 0
...
server.tool(
	'rebuild_and_reload',
	'Run the extension build command, then reload the VS Code window. Reconnects the evaluator bridge if it was active.',
	{
		build_command: z
			.string()
			.optional()
	...
	async args => {
		...
		try {
			...
			// all extensions with the newly-built code.
			if (evaluateFn) {
				try {
				...
				// Fallback: keyboard shortcut via command palette
				const mod = process.platform === 'darwin' ? 'Meta+Shift+P' : 'Control+Shift+P';
				await page.keyboard.press(mod);
				await page.waitForTimeout(400);
			...
			const parts = [`Build succeeded (${buildTime}s). VS Code reloaded.`];
