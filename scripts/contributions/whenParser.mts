...
export const ContextKeyExprType = Object.freeze({
	False: 0,
	True: 1,
	Defined: 2,
	Not: 3,
	...
	parse(input: string): ContextKeyExpression | undefined {
		if (input === '') {
			this._parsingErrors.push({
				message: 'Empty context key expression',
				offset: 0,
				lexeme: '',
				additionalInfo:
		...
		try {
			...
			if (!this._isAtEnd()) {
				...
				const additionalInfo =
					peek.type === TokenType.Str ? 'Did you forget to put && or || before the token?' : undefined;
				this._parsingErrors.push({
		...
		if (this._matchOne(TokenType.Neg)) {
			...
			switch (peek.type) {
				...
				case TokenType.LParen: {
					this._advance();
					const expr = this._expr();
					this._consume(TokenType.RParen, "closing parenthesis ')'");
					return expr?.negate();
				}
				case TokenType.Str:
					this._advance();
		...
		switch (peek.type) {
			...
			case TokenType.EOF:
				this._parsingErrors.push({
					...
					offset: peek.offset,
					lexeme: '',
					additionalInfo: 'Did you forget to put a context key?',
				});
				throw Parser._parseError;
...
export type Token =
	| { type: typeof TokenType.LParen; offset: number }
	| { type: typeof TokenType.RParen; offset: number }
	| { type: typeof TokenType.Neg; offset: number }
	| { type: typeof TokenType.Eq; offset: number; isTripleEq: boolean }
	| { type: typeof TokenType.NotEq; offset: number; isTripleEq: boolean }
	| { type: typeof TokenType.Lt; offset: number }
	| { type: typeof TokenType.LtEq; offset: number }
	| { type: typeof TokenType.Gt; offset: number }
	| { type: typeof TokenType.GtEq; offset: number }
	| { type: typeof TokenType.RegexOp; offset: number }
	| { type: typeof TokenType.RegexStr; offset: number; lexeme: string }
	| { type: typeof TokenType.True; offset: number }
	| { type: typeof TokenType.False; offset: number }
	| { type: typeof TokenType.In; offset: number }
	| { type: typeof TokenType.Not; offset: number }
	| { type: typeof TokenType.And; offset: number }
	| { type: typeof TokenType.Or; offset: number }
	| { type: typeof TokenType.Str; offset: number; lexeme: string }
	| { type: typeof TokenType.QuotedStr; offset: number; lexeme: string }
	| { type: typeof TokenType.Error; offset: number; lexeme: string }
	| { type: typeof TokenType.EOF; offset: number };

type KeywordTokenType = typeof TokenType.Not | typeof TokenType.In | typeof TokenType.False | typeof TokenType.True;
type TokenTypeWithoutLexeme =
	| typeof TokenType.LParen
	| typeof TokenType.RParen
	| typeof TokenType.Neg
	| typeof TokenType.Lt
	| typeof TokenType.LtEq
	| typeof TokenType.Gt
	| typeof TokenType.GtEq
	| typeof TokenType.RegexOp
	| typeof TokenType.True
	| typeof TokenType.False
	| typeof TokenType.In
	| typeof TokenType.Not
	| typeof TokenType.And
	| typeof TokenType.Or
...
export class ContextKeyEqualsExpr implements IContextKeyExpression {
	...
	private constructor(key: string, value: any, negated: ContextKeyExpression | null) {
		this.key = key;
		this.value = value;
		this.negated = negated;
	}
...
export class ContextKeyNotExpr implements IContextKeyExpression {
	...
	private constructor(key: string, negated: ContextKeyExpression | null) {
		this.key = key;
		this.negated = negated;
	}
...
export class ContextKeyGreaterExpr implements IContextKeyExpression {
	...
	private constructor(key: string, value: number | string, negated: ContextKeyExpression | null) {
		this.key = key;
		this.value = value;
		this.negated = negated;
	}
	...
	public evaluate(context: IContext): boolean {
		...
		return parseFloat(context.getValue(this.key) as any) > this.value;
	}
...
export class ContextKeyNotRegexExpr implements IContextKeyExpression {
	...
	private constructor(actual: ContextKeyRegexExpr) {
		this._actual = actual;
	}
...
export class ContextKeyAndExpr implements IContextKeyExpression {
	...
	private constructor(expr: ContextKeyExpression[], negated: ContextKeyExpression | null) {
		this.expr = expr;
		this.negated = negated;
	}
