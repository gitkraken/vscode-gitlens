...
async function fetchAuditIssues(params: AuditQueryParams): Promise<GitHubIssue[]> {
	...
	const issues = await fetchSinglePage(query, startCursor);
...
async function fetchSinglePage(query: string, cursor: string | null): Promise<GitHubIssue[]> {
	const raw = await execGraphQL(query, cursor);
	const response = JSON.parse(raw);
