...
export async function extractTarGz(tarball, destination) {
	...
	for (let offset = 0; offset + 512 <= archive.length;) {
		...
		// Reject entries (e.g. `../../etc/passwd`) that resolve outside `destination` before any write.
		if (!resolve(target).startsWith(destinationRoot)) {
			throw new Error(`Refusing to extract '${relativePath}' outside of destination '${destination}'`);
		}
