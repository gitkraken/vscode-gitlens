...
export class GraphWebview extends WebviewBase<State> {
		...
		if (search == null || search.comparisonKey !== getSearchQueryComparisonKey(e.search)) {
			...
			try {
				search = await this.repository.searchCommits(e.search, {
					...
					includes: {
						remotes: !commitFilter?.hideRemotes,
						stashes: !commitFilter?.hideStashes,
						tags: !commitFilter?.hideTags,
					},
	...
	private onCommitFilterSelectionChanged(e: UpdateCommitFilterSelectionParams) {
		...
		let commitFilter = storedCommitFilter?.[this.repository.id];
		commitFilter = { ...commitFilter, ...e.commitFilter };
		storedCommitFilter = updateRecordValue(storedCommitFilter, this.repository.id, commitFilter);
