...
import { createBufferedCallback, createEventSubscription } from '../rpc/eventVisibilityBuffer.js';
...
export class CommitDetailsWebviewProvider implements WebviewProvider<State, State, CommitDetailsWebviewShowingArgs> {
	...
	private async onSuggestChanges(
		repoPath: string,
		params: {
			title: string;
			description?: string;
		...
		const prEntityId = await this.getEncodedPrEntityId(repoPath);
		...
		try {
			...
			void this.trackCreateCodeSuggestion(draft, createFileCount, repoPath);
		} catch (ex) {
			debugger;
	...
	private onCreatePatchFromWip(params: { changes: WipChange; checked: boolean | 'staged' }) {
	...
	// Note: showCodeSuggestion is now handled via RPC with the draft object passed directly
	...
	private async onReachabilityRequest(
		repoPath: string,
		sha: string,
		signal?: AbortSignal,
		...
		try {
			signal?.throwIfAborted();
			const result = await this.container.git.getRepositoryService(repoPath).commits.getCommitReachability?.(sha);
	...
	private onNavigateStack(params: { direction: 'back' | 'forward' }): {
		count: number;
		position: number;
		hint?: string;
	} {
		...
		if (commit != null) {
			...
			// Forward commit selection to webview - it will fetch the commit data via RPC
			this._onCommitSelected.fire({
				repoPath: commit.repoPath,
				sha: commit.ref,
				passive: false,
		...
		return this.getNavigationStack();
	}
	...
	/**
	 * Get basic autolinks for a commit + message formatted with autolink patterns.
	 * Resolves remote for URL patterns; does NOT call enrichment APIs.
	 */
	private async getCommitAutolinksData(commit: GitCommit) {
	...
	@trace({ args: false })
	private async getEnrichedState(commit: GitCommit, remote: GitRemote | undefined): Promise<CommitEnrichedResult> {
	...
	getRpcServices(buffer?: EventVisibilityBuffer, tracker?: SubscriptionTracker): CommitDetailsServices {
		const base = createCommonServices(
			this.container,
			this.host,
			context => {
		...
		return proxyServices({
			...
			// ============================================================
			// Git: base service + view-specific overrides
			// ============================================================
			git: Object.assign(base.git, {
				// Override with view-specific commit selection (different event shape,
				// includes search context, passive flag, attachedTo filtering).
				// Replays the cached event on subscribe so the webview doesn't miss
				// selections that fired before the subscription was set up.
				onCommitSelected: (callback: (event: CommitSelectionEvent) => void) => {
					...
					// This fires directly (not buffered) since it runs at subscribe time
					// when the webview is visible.
					const commitRef = this.resolveCurrentCommitRef();
					if (commitRef != null) {
						callback({
							repoPath: commitRef.repoPath,
				...
				getCommitAutolinks: async (repoPath: string, sha: string, signal?: AbortSignal) => {
					signal?.throwIfAborted();
					const commit = await this.container.git.getRepositoryService(repoPath).commits.getCommit(sha);
					if (commit == null) return undefined;
					signal?.throwIfAborted();
					return this.getCommitAutolinksData(commit);
				},

				getCommitEnriched: async (repoPath: string, sha: string, signal?: AbortSignal) => {
					signal?.throwIfAborted();
					const commit = await this.container.git.getRepositoryService(repoPath).commits.getCommit(sha);
					if (commit == null) return undefined;
					signal?.throwIfAborted();
					const remote = await this.container.git
						...
						.remotes.getBestRemoteWithIntegration({ includeDisconnected: true });
					signal?.throwIfAborted();
					return this.getEnrichedState(commit, remote);
				},
				...
				getAssociatedPullRequest: async (repoPath: string, signal?: AbortSignal) => {
					signal?.throwIfAborted();
					const repo = this.container.git.getRepository(repoPath);
					if (repo == null) return undefined;
				...
				getCommitReachability: (repoPath: string, sha: string, signal?: AbortSignal) =>
					this.onReachabilityRequest(repoPath, sha, signal),
			}),

			// ============================================================
			// AI: base service + view-specific operations
			// ============================================================
			ai: Object.assign(base.ai, {
			...
			// ============================================================
			// Actions: view-specific UI actions (file ops, commit ops, PR ops)
			// ============================================================
			actions: {
				...
				executeFileAction: async (file, showOptions?, ref?) => {
					await this.onShowFileActions({
						...file,
						ref: ref,
				...
				openFile: async (file, showOptions?, ref?) => {
					await this.openFile({
						...file,
						ref: ref,
						showOptions: showOptions,
				...
				openFileOnRemote: async (file, ref?) => {
					await this.openFileOnRemote({ ...file, ref: ref });
				},

				openFileCompareWorking: async (file, showOptions?, ref?) => {
					await this.openFileChangesWithWorking({
						...file,
						ref: ref,
						showOptions: showOptions,
					});
				...
				openFileComparePrevious: async (file, showOptions?, ref?) => {
				...
				openPullRequestChanges: async (repoPath, prRefs) => {
				...
				openPullRequestComparison: (repoPath, prRefs) => {
					const refs = getComparisonRefsForPullRequest(repoPath, prRefs);
					void this.container.views.searchAndCompare.compare(refs.repoPath, refs.head, refs.base);
					return Promise.resolve();
				},

				openPullRequestOnRemote: async prUrl => {
					await executeCommand<OpenPullRequestOnRemoteCommandArgs, void>('gitlens.openPullRequestOnRemote', {
						pr: { url: prUrl },
					});
				},

				openPullRequestDetails: async (repoPath, _prId, _prProvider) => {
					const status = await this.container.git.getRepositoryService(repoPath).status.getStatus();
					if (status?.branch == null) return;

					const branch = await this.container.git
						.getRepositoryService(repoPath)
						.branches.getBranch(status.branch);
					const pr = await branch?.getAssociatedPullRequest();
					if (pr == null) return;
			...
			// ============================================================
			// Navigation: view-specific (no generic equivalent)
			// ============================================================
			navigation: {
				getInitialContext: () =>
					Promise.resolve({
				...
				onShowWip: createEventSubscription<{ repoPath?: string; inReview: boolean }>(
					buffer,
					'showWip',
					'save-last',
			...
			// ============================================================
			// Drafts: view-specific (no generic equivalent)
			// ============================================================
			drafts: {
				getCodeSuggestions: async (repoPath, signal?) => {
					signal?.throwIfAborted();
					const repo = this.container.git.getRepository(repoPath);
					if (repo == null) return [];
					...
					const suggestions = await this.getCodeSuggestions(pr, repo);
				...
				createPatchFromWip: (changes, checked) => {
					this.onCreatePatchFromWip({ changes: changes, checked: checked });
					return Promise.resolve();
				},

				suggestChanges: async params => {
					await this.onSuggestChanges(params.repoPath, params);
				},

				showCodeSuggestion: draft => {
