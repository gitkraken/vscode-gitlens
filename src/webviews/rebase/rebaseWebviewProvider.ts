...
export class RebaseWebviewProvider implements Disposable {
	...
	private async onResolveAllConflicts(params: IpcParams<typeof ResolveAllConflictsCommand>): Promise<void> {
		...
		await Promise.all(
			toDelete.map(async p => {
				try {
					await workspace.fs.delete(Uri.joinPath(Uri.file(this.repoPath), p), { useTrash: false });
				} catch {
					// File may already be gone; continue to stage
				}
			}),
		);

		const toStage = [...toCheckoutOurs, ...toCheckoutTheirs, ...toDelete];
		if (toStage.length) {
			try {
	...
	private async applyConflictResolution(
		...
		switch (action) {
			case 'delete': {
				const uri = Uri.joinPath(Uri.file(this.repoPath), path);
				try {
