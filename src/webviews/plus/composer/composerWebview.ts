...
import { createComposeToolsIntegration } from '@env/composer.js';
...
export class ComposerWebviewProvider implements WebviewProvider<State, State, ComposerWebviewShowingArgs> {
	...
	// Compose-tools integration (exercise-only). Node-only — the webworker build
	// resolves `@env/composer.js` to a browser stub that returns undefined, which
	...
	private readonly _composeTools: ComposeToolsIntegration | undefined;
	/** Cache key returned by integration.generatePlan — consumed by a future library-backed onFinishAndCommit. */
	...
	constructor(
		...
		this._composeTools = createComposeToolsIntegration(container);
	...
	private async onGenerateCommits(params: IpcParams<typeof GenerateCommitsCommand>): Promise<void> {
		...
		try {
			...
			if (useLibraryRoute && this._composeTools != null && this._currentRepository != null) {
				...
				let librarySource: import('../../../plus/composer/composeToolsIntegration.js').ComposerSource;
				...
				try {
					...
					result = {
						...
						// Library path doesn't produce an AIConversation — the "try again with
						// same hunks" path won't resume a prior library session until Gap 11
						// (resumable sessions) lands. Fine for the MVP: a second compose re-runs
