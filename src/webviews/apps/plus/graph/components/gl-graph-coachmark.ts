...
export class GlGraphCoachMark extends SignalWatcher(LitElement) {
	...
	/** Set by the ✕ button only. Escape and blur count as incidental, which costs the mark nothing but
	 *  its next-session force-open. */
	...
	override updated(changedProperties: PropertyValues): void {
		...
		if (this.autoShow) {
			if (!forceOpenedThisSession.has(this.mark) && this._seen?.has(this.mark) === false) {
		...
		return html`<gl-popover trigger="manual" placement=${this.placement} @gl-popover-hide=${this.onPopoverHide}>
				...
				>
					<div class="coachmark__header">
						<span id="coachmark-title" class="coachmark__title">${content.title}</span>
