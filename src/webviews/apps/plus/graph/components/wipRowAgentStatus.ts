 ...
 *  `ended` sessions are a deliberate divergence: they're dropped outright here regardless of
 *  age (see `pickWipRowAgentStatus`), while the host's agents-scope filter still keeps recently-
 *  ended sessions — a branch whose agent just finished is exactly what that scope is for. */
...
const categoryPriority: Record<AgentSessionCategory, number> = {
	...
	working: 1,
	idle: 2,
	// Unreachable — ended sessions are filtered out before this runs.
	ended: 3,
};
...
/** Drops `ended` sessions outright and `idle` sessions older than `wipRowAgentIdleThresholdMs`
