		...
		if (this.graphState.trusted === false) {
			return html`
				<div class="scroller scrollable">
					<div class="container" role="group" aria-label=${l10n.t('Untrusted workspace')}>
						<div class="hero">
							<div class="icon"><code-icon icon="workspace-untrusted" style="font-size: 4rem;"></code-icon></div>
							<h2 class="title">${l10n.t('Untrusted workspace')}</h2>
							<p class="description">
								${l10n.t(
									"GitLens can't open repositories while this workspace is in Restricted Mode. Trust this workspace to visualize its history, branches, and commits in the Commit Graph.",
		...
		if (this.graphState.hasUnsafeRepositories) {
			return html`
				<div class="scroller scrollable">
					<div class="container" role="group" aria-label=${l10n.t('Unsafe repository')}>
						<div class="hero">
							<div class="icon"><code-icon icon="warning" style="font-size: 4rem;"></code-icon></div>
							<h2 class="title">${l10n.t('Unsafe repository')}</h2>
							<p class="description">
								${l10n.t(
									'Unable to open any repositories — Git blocked them as potentially unsafe, because their folders are not owned by the current user. Mark them as safe in Source Control to visualize their history, branches, and commits in the Commit Graph.',
			...
			<div class="scroller scrollable">
				<div class="container">
					<div class="hero">
						...
						<h1 class="title">${l10n.t('No repository open')}</h1>
						<p class="description">${l10n.t('Choose how you want to get started.')}</p>
					</div>
					<div class="groups">
						<div class="group" role="group" aria-labelledby="label-get-started">
							<h2 class="group__label" id="label-get-started">${l10n.t('Get started')}</h2>
							${when(
								...
								() =>
									this.renderAction({
										...
										icon: 'globe',
										title: l10n.t('Open a Remote Repository'),
										description: l10n.t('Work with a repository without cloning it locally'),
									}),
								() => html`
									${this.renderAction({
										href: 'command:workbench.action.files.openFolder',
										icon: 'folder-opened',
										title: l10n.t('Open a Folder'),
										description: l10n.t('Browse for a local folder or repository'),
									})}
									${this.renderAction({
										href: 'command:git.clone',
										icon: 'repo-clone',
										title: l10n.t('Clone a Repository'),
										description: l10n.t('Get a remote repository onto your machine'),
									})}
									${this.renderAction({
										href: 'command:git.init',
										icon: 'new-folder',
										title: l10n.t('Start a New Project'),
										description: l10n.t('Create a folder and initialize a repository'),
									})}
								`,
							)}
						</div>
