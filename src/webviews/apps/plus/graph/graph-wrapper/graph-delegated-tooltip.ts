...
import type { GraphRefsMetadata } from '../../../../plus/graph/protocol.js';
import type { GlPopover } from '../../../shared/components/overlays/popover.js';
import { renderIssueTooltipCard, renderPullRequestTooltipCard } from './adornments/refAdornmentProvider.js';
import type { RowMarkers } from './graph-scroll-markers.js';
...
export type DelegatedTooltipHost = {
	...
	scrollMarkerRows(): readonly RowMarkers[];
...
export class DelegatedTooltipController implements ReactiveController {
	...
	private _entries: readonly { icon: string; label: string }[] = [];
	...
	showForTarget(target: HTMLElement): void {
		...
		if (rowAttr != null) {
			const row = this.deps.scrollMarkerRows().find(r => r.index === Number(rowAttr));
			const entries =
			...
			this.showList(target, entries, 'left', 60);
			return;
		}
	...
	private _resolveRefMetadataTooltip(type: string, refId: string | undefined): TemplateResult | undefined {
		...
		const metadata = this.deps.refsMetadata()?.[refId];
	...
	render(): TemplateResult {
		return html`<gl-popover
			...
			class="gl-graph__tooltip${this._entries.length > 0 ? ' is-list' : ''}"
			trigger="manual"
			placement=${this._placement}
			?arrow=${this._entries.length === 0}
			.distance=${this._entries.length > 0 ? 4 : 6}
			.anchor=${this._anchor}
			.open=${this._open}
		>
			<span slot="anchor"></span>
			<span slot="content" class="gl-graph__tooltip-content${this._entries.length > 0 ? ' is-list' : ''}"
				>${
					this._content ??
