/**
 * Signal-based state management for the Timeline webview.
 *
 * State is instance-owned: the root component creates a `TimelineState` via
 * `createTimelineState()` and passes it to actions as a parameter.
 * No module-level singletons.
 *
 * State Categories:
 * - Persisted (survive hide/show/refresh): period, showAllBranches, sliceBy, scope
 * - Ephemeral (reset each session): displayConfig, repository, repositories, access, error
 * - Derived (computed from above): allowed, head, base, effectiveSliceBy, etc.
 * - Resource (external data): dataset — managed by createResource() in timeline.ts
 */
...
export function createTimelineState(storage?: HostStorage) {
	...
	// ── Persisted state (survive hide/show/refresh) ──

	const period = persisted<TimelinePeriod>('period', '1|Y' as TimelinePeriod);
	const showAllBranches = persisted('showAllBranches', false);
	const sliceBy = persisted<TimelineSliceBy>('sliceBy', 'author' as TimelineSliceBy);
