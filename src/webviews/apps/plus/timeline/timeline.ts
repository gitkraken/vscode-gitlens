...
export class GlTimelineApp extends SignalWatcherWebviewApp {
	...
	private async _onRpcReady(services: Remote<TimelineServices>): Promise<void> {
		...
		const datasetResource = createResource<TimelineDatasetResult | undefined>(async _signal => {
			const currentScope = s.scope.get();
			if (currentScope == null) return undefined;

			return services.getDataset(currentScope, {
				period: s.period.get(),
		...
		const actions = new TimelineActions(s, services, datasetResource);
		this._actions = actions;

		// Start auto-persistence before any state changes from host
		this._stopAutoPersist = s.startAutoPersist();

		// Resolve domain sub-services that have events
		const [git, subscription, config] = await Promise.all([services.git, services.subscription, services.config]);
		...
		this._unsubscribeEvents = await setupSubscriptions(
			services,
			{ git: git, subscription: subscription, config: config },
			subActions,
		);
	...
	// ============================================================
	// Event handlers (thin DOM wrappers → actions)
	// ============================================================
	...
	private renderChart() {
		...
		const dataPromise = datasetResult?.dataset != null ? Promise.resolve(datasetResult.dataset) : undefined;
