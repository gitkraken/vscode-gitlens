 ...
 * - git.onCommitSelected (view-specific override)
 * - git.onRepositoryChanged
...
interface SubscriptionServices {
	readonly git: Awaited<Remote<CommitDetailsServices>['git']>;
	...
	readonly navigation: Awaited<Remote<CommitDetailsServices>['navigation']>;
}
...
export function setupSubscriptions(
	...
	return subscribeAll([
		() =>
			services.git.onCommitSelected((event: CommitSelectionEvent) => handleCommitSelected(state, event, actions)),
		() =>
			services.git.onRepositoryChanged((event: RepositoryChangeEventData) =>
		...
		() => services.navigation.onShowWip((event: ShowWipEvent) => handleShowWip(state, event, actions)),
	]);
}
...
function handleCommitSelected(
	...
	// If in WIP mode and not pinned, switch to commit mode
	if (state.mode.get() === 'wip' && !isPinned) {
