...
export class GlCommitDetailsApp extends SignalWatcherWebviewApp {
	...
	private async _onRpcReady(services: Remote<CommitDetailsServices>): Promise<void> {
		...
		const [git, commands, config, storage, ai, subscription, integrations, actions, navigation, drafts, telemetry] =
			await Promise.all([
				services.git,
				services.commands,
				services.config,
		...
		const resources: CommitDetailsResources = {
			commit: createResource((signal, repoPath: string, sha: string) => git.getCommit(repoPath, sha, signal)),
			wip: createResource((signal, repoPath: string | undefined) => git.getWipChanges(repoPath, signal)),
			reachability: createResource<GitCommitReachability | undefined>(async signal => {
				const commit = s.currentCommit.get();
				if (commit == null) return undefined;
				const result = await git.getCommitReachability(commit.repoPath, commit.sha, signal);
				if (result.error) {
			...
			explain: createResource<ExplainState | undefined>(async signal => {
				...
				if (commit == null) return undefined;
				try {
					const result = await ai.explainCommit(commit.repoPath, commit.sha, signal);
					if (result.error) {
			...
			generate: createResource<GenerateState | undefined>(async signal => {
				...
				try {
					const result = await ai.generateDescription(repoPath, signal);
					if (result.error) {
						return { error: { message: result.error.message ?? 'Error retrieving content' } };
					}
		...
		this._unsubscribeEvents = await setupSubscriptions(
			s,
			{ git: git, config: config, integrations: integrations, navigation: navigation },
			this._actions,
		);
	...
	private setupDomListeners(): void {
		...
		const onVisibilityChange = (): void => {
			...
			// Visibility restored — re-fetch WIP if applicable
			if (s.mode.get() !== 'wip' || s.loading.get()) return;
