 ...
 * - homeStateContext — repositories, preview, overview filter, services
 */

import type { Remote } from '@eamodio/supertalk';
import { createContext } from '@lit/context';
import type { HomeInitialContext, HomeServices, PreviewState } from '../../home/homeService.js';
...
export function createHomeState(storage?: HostStorage) {
	...
	return {
		...
		previewState: signal<PreviewState>({
			previewEnabled: false,
			aiEnabled: false,
			experimentalComposerEnabled: false,
		}),
		/** Initial context (populated once on connect). */
		initialContext: signal<HomeInitialContext | undefined>(undefined),
