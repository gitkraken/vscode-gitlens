...
export class WebviewIntegrationsService {
	/**
	...
	/**
	 * Check if any cloud integration is currently connected.
	 */
	async hasAnyConnected(): Promise<boolean> {
		const states = await getIntegrationStates(this.container);
		return states.some(i => i.connected);
	}
}
