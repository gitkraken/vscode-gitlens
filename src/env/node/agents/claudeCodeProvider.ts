...
const sessionsDir = join(tmpdir(), 'gitkraken', 'gitlens', 'agents', 'sessions');
...
export class ClaudeCodeProvider implements AgentSessionProvider {
	...
	private async ensureIpcServer(workspacePaths: string[]): Promise<void> {
		...
		try {
			...
			// Restore sessions from disk after handlers are registered
			await this.syncSessionsFromDisk();

			// Query peers for sessions they know about (fire-and-forget)
			void this.queryPeersForSessions();

			// Start periodic sync to clean up stale files and remove phantom sessions
			if (this._staleCheckTimer != null) {
				clearInterval(this._staleCheckTimer);
			}
			this._staleCheckTimer = setInterval(() => {
				void this.syncSessionsFromDisk();
	...
	private handleSessionEvent(event: AgentSessionEvent): void {
		...
		switch (event.event) {
			case 'session-start': {
				const index = this.ensureSession(event.sessionId, event.pid, event.matchedWorkspacePath, isLocal);
			...
			case 'subagent-start': {
				const parentIdx = this.ensureSession(event.sessionId, event.pid, event.matchedWorkspacePath, isLocal);
	...
	private updateSessionStatus(
		...
		const index = this.ensureSession(sessionId, pid, matchedWorkspacePath, isLocal);
	 ...
	 */
	private ensureSession(sessionId: string, pid?: number, workspacePath?: string, isLocal?: boolean): number {
	...
	/**
	 * Reads session files from disk, cleaning up stale/malformed files,
	 * adding newly discovered alive sessions to memory, and removing
	 * in-memory sessions whose processes are no longer alive.
	 */
	private async syncSessionsFromDisk(): Promise<void> {
		...
		for (const file of files) {
			...
			try {
				...
				data = JSON.parse(raw) as SessionFileData;
			} catch {
