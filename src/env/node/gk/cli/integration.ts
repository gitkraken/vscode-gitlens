...
export class GkCliIntegrationProvider implements Disposable {
	...
	private async start() {
		...
		server.registerHandler('ping', () => Promise.resolve({ stdout: `{ version: '${this.container.version}' }` }));
		...
		try {
			...
			if (workspaceFolders != null && workspaceFolders.length > 0) {
				...
				this._discoveryFilePath = await createDiscoveryFile(server.ipcAddress, workspacePath, workspacePaths);
			}
		} catch (error) {
 ...
 */
function getDiscoveryFilePath(workspacePath: string): string {
...
async function createDiscoveryFile(address: string, workspacePath: string, workspacePaths: string[]): Promise<string> {
	const discoveryDir = join(tmpdir(), 'gitkraken', 'gitlens');
	const filePath = getDiscoveryFilePath(workspacePath);
