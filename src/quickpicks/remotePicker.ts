	...
	export async function show(
		...
		if (remotes.length === 0) {
			...
			const pickOpts: Parameters<typeof createRemoteQuickPickItem>[2] = {
				upstream: true,
				buttons: options?.setDefault ? [QuickCommandButtons.SetRemoteAsDefault] : undefined,
			};
			...
			const pick = await new Promise<RemoteQuickPickItem | undefined>(resolve => {
				disposables.push(
					...
					quickpick.onDidTriggerItemButton(async e => {
						if (e.button === QuickCommandButtons.SetRemoteAsDefault) {
