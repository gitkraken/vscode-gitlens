...
| Commit Graph    | **Hybrid** | RPC for everything except the rows data plane and its adjacent planes — auxiliary services, search, agent sessions, gating/access, one-shot pushes, request/response planes, and all write planes (filters, columns, config); legacy IPC carries rows, selection, viewport feeds, WIP, and navigation pushes (32 IPC types, `plus/graph/protocol.ts`) |
...
the write planes (`filters`, `columns`, `configuration` in `plus/graph/graphService.ts`). Rows,
selection, viewport feeds (avatars/refs-metadata), WIP, and host→webview navigation pushes still
travel over legacy IPC. The ledger-diffed splice channel in
