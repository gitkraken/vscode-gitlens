...
| Commit Graph    | **Hybrid** | RPC for everything except the rows data plane and its adjacent planes — auxiliary services, search, agent sessions, gating/access, one-shot pushes, request/response planes, all write planes (filters, columns, config), the WIP plane, selection, navigation pushes, and row actions; legacy IPC carries only the rows plane and its viewport avatar/refs-metadata feeds (11 IPC types, `plus/graph/protocol.ts`) |
...
`plus/graph/graphService.ts`. Only the rows plane (paging, splices, sync) and its viewport
avatar/refs-metadata feeds still travel over legacy IPC. The ledger-diffed splice channel in
