...
| Commit Graph    | **Hybrid** | RPC for everything except the rows data plane and its adjacent planes — auxiliary services, search, agent sessions, gating/access, one-shot pushes, request/response planes, all write planes (filters, columns, config), and the WIP plane; legacy IPC carries rows, selection, viewport avatar/refs-metadata feeds, and navigation pushes (25 IPC types, `plus/graph/protocol.ts`) |
...
drafts, watch lifecycle, and the tick/probe working-tree events, all in
`plus/graph/graphService.ts`). Rows, selection, viewport feeds (avatars/refs-metadata), and
host→webview navigation pushes still travel over legacy IPC. The ledger-diffed splice channel in
