...
- `pnpm run check:l10n` also ratchets each shipped locale as a whole against `scripts/l10n-coverage-baseline.json`, catching damage no per-message check can see. A locale must ship both of its catalogs. The count of entries byte-identical to their English source may not grow, and neither may the count of plural branches the locale can never select — both are what a mis-keyed or mechanically imported catalog looks like. Coverage may slip by the baseline's `coverageSlack` (2 points), which at today's catalog size absorbs roughly 140 new English messages before a build reddens, while a larger drop means the catalogs need re-importing. Run `pnpm run check:l10n:update` to re-pin the numbers once a change to them is a decision rather than an accident — a merge that brings English strings back, a prune that drops many entries, and a newly added locale all need it.
- `pnpm run check:l10n:packaged` asserts the VSIX would carry every shipped locale's catalogs and none of the generated `qps-ploc` ones. Nothing else covers that: `.vscodeignore` only applies at `vsce package`, and the E2E suite runs the extension from a directory, where it has no effect at all.
