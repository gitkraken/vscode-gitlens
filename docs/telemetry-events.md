...
```typescript
{
  ...
  // Groups every request of one AI session — the whole user-facing task. Set by conflict resolution (`type: 'resolveConflicts'`) and by Graph compose (`type: 'commits'`); absent on every other feature, whose requests are one-per-task anyway. Counting distinct IDs (filtered by `type`) is how usage is measured for both — the event count itself can't be, since one session is many round-trips: an agentic loop for resolution, and the library's validation retries plus the user's refines for compose. For per-operation counts use `autoRebase/step/resolved` (automatic resolution), `graphDetails/resolve/generateResolutions/completed` (the resolve panel), and `graphDetails/compose/applyPlan/completed` (compose). Two caveats. An escalated rebase's ID is deliberately adopted by the resolve panel that finishes it, so a single ID can carry requests from both paths and distinct-ID counts can't be split cleanly on `source.detail`. And a compose ID survives a generate that errored or was cancelled, so the user's retry continues it — a distinct ID counts one compose session, not one plan produced.
