...
function fileUriJson(fsPath: string): { $mid: number; fsPath: string; external: string; path: string; scheme: string } {
	// VS Code revives the URI from `path`, which is the URI path (`/C:/...` on Windows), not the native `fsPath`
	const url = pathToFileURL(fsPath);
	return { $mid: 1, fsPath: fsPath, external: url.href, path: decodeURIComponent(url.pathname), scheme: 'file' };
}
