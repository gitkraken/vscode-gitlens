...
test(
	...
	async ({ vscode }, testInfo) => {
		...
		const scroll = await root.evaluate(async () => {
			const scroller = document.querySelector<HTMLElement>('gl-commit-graph lit-virtualizer');
		...
		const report = {
			...
			// Each entry is the byte length of the RPC frame that carried that rows update (measured off
			// the wire, not a re-serialization of the decoded payload) — see `receivedBytes` in
			// stateProvider.ts's `applyRowsPayload`. A frame can batch multiple RPC messages together, in
			// which case an entry is the whole frame's size, not that rows update's share of it.
