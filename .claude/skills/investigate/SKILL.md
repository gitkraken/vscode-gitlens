...
- `--from-report` — Reads a triage decisions JSON. If path omitted, uses most recent `*-DECISIONS.json` in `.triage/reports/`.
...
1. Read the decisions JSON file specified (or find the most recent `*-DECISIONS.json` in `.triage/reports/`)
